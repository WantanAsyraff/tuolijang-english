(function () {
  ...
  class ChatIframe {
    ...
    constructor(options) {
      ...
      // 懒加载状态：iframe 的 src 默认不写入，首次"放大/打开应用"时才加载，
      // 避免一进宿主页就拉取模型与聊天记录数据影响首屏速度
      this.iframeSrcLoaded = false;
      // iframe 内 app 是否已挂载消息监听（收到 iframe-ready 握手后置为 true）
      this.iframeReady = false;
      // iframe 就绪前积压的父→子消息，握手完成后统一补发，规避加载时序竞态
    ...
    createIframe() {
      const { zIndex, floatIcon, defaultShow, permissions } = this.options;
      ...
      <div id="${containerId}" 
        ...
        <iframe allow="${permissions}" class="chat-iframe"></iframe>
    ...
    // 懒加载入口：首次需要展示 iframe 内容时才写入 src，触发聊天 app 加载
    ensureIframeLoaded() {
      if (this.iframeSrcLoaded) return;
    ...
    sendMessageToIframe(action, data) {
      // iframe 尚未加载（用户从未点开过悬浮球），刷新应用列表等被动消息直接忽略，
      // 不应为此触发懒加载；真正需要展示的入口会显式调用 ensureIframeLoaded()
      if (!this.iframeSrcLoaded) return;

      // iframe 已开始加载但 app 还未挂载监听时，先把消息排队，收到握手后再补发
      if (!this.iframeReady) {
    ...
    // 收到 iframe 内 app 的就绪握手后，补发此前积压的消息
    handleIframeReady() {
      this.iframeReady = true;
      const pending = this.pendingMessages;
      this.pendingMessages = [];
      pending.forEach(({ action, data }) => this.sendMessageToIframe(action, data));
    }
