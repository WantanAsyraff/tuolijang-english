<template>
  <div class="page-account" :class="{ 'lang-en': language === 'en' }">
    ...
    <!-- 企微登录 -->
    <authDialog ref="authDialog" @workloginOk="workloginOk"></authDialog>
  </div>
</template>
...
import loginAccountImg from '@/assets/images/login-account.png'
import loginCodeImg from '@/assets/images/login-code.png'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      captchatImg: '',
      loginForm: {
        account: localStorage.getItem('lastLoginAccount') || '', // 回显上次登录账号
        password: '',
        key: '',
        captcha: '',
        verification_code: '',
  ...
  created() {
    ...
    if (this.$route.query) {
      ...
      if (urlObj.searchParams.get('code')) {
        ...
        this.wxCode = codeValue
        this.wxWorkLogin(codeValue)
      }
    }
  ...
  methods: {
    ...
    normalizeLoginPhone() {
      if (this.loginForm.phone) {
        this.loginForm.phone = helper.normalizePhone(this.loginForm.phone)
      }
    },
    // 站点配置
    site() {
    ...
    getWorkId() {
      getCorpConfig().then((res) => {
        ...
        localStorage.setItem('wxConfig', JSON.stringify(wxConfig))
      })
    },
    // 打开企业微信登录
    openqiweiFn() {
      this.$refs.authDialog.open('login')
    },
    tabChange() {
    ...
    wxWorkLogin(code) {
      workAuthLogin({ code: code }).then((res) => {
        this.workloginOk(res.data)
      })
    },
    workloginOk(val) {
      this.$store.commit('user/SET_TOKEN', val.token)
      this.$store.commit('user/SET_REFRESH_TOKEN', val.refresh_token || '')
    ...
    async loginAfter() {
      if (this.$store.getters.isLogin) {
        // 登录成功后缓存当前登录账号，退出后用于回显
        const loginAccount = this.activeName === 'cmsLogin' ? this.loginForm.phone : this.loginForm.account
        if (loginAccount) {
          localStorage.setItem('lastLoginAccount', loginAccount)
        }
        ...
        let work_member_id = getStorageJson('userInfo', {}).work_member_id

        // 企业微信绑定弹窗

        if (this.corpid && !work_member_id) {
          await this.$authDialog.open('')
        }
