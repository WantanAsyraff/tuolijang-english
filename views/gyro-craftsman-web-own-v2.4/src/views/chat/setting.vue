        ...
        <el-col :span="8">
          <div class="content p0">
            <modelForm
              ...
              :info="info"
              tab-name="modelForm"
              @update-preview="handleUpdatePreviewState"
            >
            </modelForm>
          </div>
        </el-col>
...
export default {
  ...
  data() {
    ...
    return {
      previewIframeUrl: AiEmbeddedManager.getPreviewIframeUrl(id, {
  ...
  methods: {
    handleUpdatePreviewState(state) {
      ...
      AiEmbeddedManager.updateAiAppPreviewState(this.$refs.previewIframe, nextState)
    },
    getInfo(id) {
    ...
    sendToServer(data, val) {
      chatPutApplicationsApi(this.id, data).then((res) => {
        ...
        this.loading = false
        AiEmbeddedManager.refreshFloatEntryAppList()
      })
    },
    publishAndSave(data) {
