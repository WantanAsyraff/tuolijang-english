...
export default {
    ...
    return {
      ...
      search: [
        ...
        },
        {
          ...
          pickerOptions: this.getOnlyRangePickerOption(),
  ...
  methods: {
    ...
    getOnlyRangePickerOption() {
      return {
        ...this.$pickerOptionsTimeEle,
        shortcuts: this.$pickerOptionsTimeEle.shortcuts.slice(2)
      }
    },
    confirmData(data) {
      if (data == 'reset') {
    ...
    init() {
      this.optionData = {
        tooltip: {
          ...
          confine: true,
          enterable: true,
          extraCssText: 'max-height:480px;overflow-y:auto;',
          axisPointer: {
            type: 'line',
            lineStyle: {
              color: '#CCCCCC'
          ...
          formatter: (params) => {
            ...
            const title = this.formatChartAxisText(params[0].axisValueLabel || params[0].name)
            let rows = ''
            params.forEach((item) => {
            ...
            // 数据量大于 20 条时改为两列展示
            if (params.length > 20) {
              rows = `<div style="column-count:2;column-gap:40px">${rows}</div>`
            }
            return `<div style="padding-left:10px">${title}${rows}</div>`
          }
        },
        legend: {
          data: [],
      ...
      this.optionData.xAxis[0].data = (this.tableData.xAxis || []).map((item) => this.formatChartAxisText(item))
...
::v-deep .from-s .flex-row .el-form-item {
  margin-right: 10px;
  margin-left: 0;
}
// 考核统计 tooltip 滚动条美化（细、半透明、悬浮）
// echarts 4.2.1 的 tooltip.className 不生效，改用结构选择器命中 #statistics-con 内的 tooltip dom
::v-deep #statistics-con > div {
