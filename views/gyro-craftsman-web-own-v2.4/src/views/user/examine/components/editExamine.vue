  ...
  methods: {
    ...
    approveApply(id, val, type) {
      approveApplyFormApi(id, this.parameterData).then((res) => {
        ...
        //       const props = per.props;
        //       const isDisabledField = props && props.disabled;
        //       if (!isDisabledField) {
        //         delete per.value
        //       }
    ...
    approveApplySave(id) {
      ...
      this.$refs.fc.$f.submit((formData) => {
        ...
        const data = {
          ...
          link_id: this.parameterData.remind_id || 0
        }
    ...
    attendanceAbnormalRecord(id) {
      this.rules.forEach((item) => {
        if (item.symbol == 'attendanceExceptionDate' && item.options.length > 0) {
          let option = item.options.filter((it) => it.value == id)
          this.tryYiChangOptions = option[0].record
        }
        if (item.symbol == 'attendanceExceptionRecord' && this.tryYiChangOptions.length > 0) {
          item.options = this.tryYiChangOptions
          item.value = ''
        }
      })
    }
...
::v-deep .el-form-item--mini.el-form-item:last-child {
  margin-bottom: 0px;
}
::v-deep .el-form-item {
  margin-bottom: 20px;
