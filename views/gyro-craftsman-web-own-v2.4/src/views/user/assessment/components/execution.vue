...
export default {
  ...
  data() {
    return {
      ...
      max: getStorageJson('enterprise', {}).maxScore,
      message1: '驳回申诉',
      message2: '绩效申诉',
      message3: '请输入驳回理由',
      message4: '请输入申诉理由',
  ...
  methods: {
    ...
    handleScore(index) {
      ...
      this.assessInfo.max = this.tableData.reduce((sum, item) => sum + item.ratio, 0)
    },
    // 删除维度
...
.current-list > div:first-child {
  flex-shrink: 0;
}

.current-list > div:last-child {
  flex: 1;
  display: flex;
  flex-direction: column;
  min-height: 0;
