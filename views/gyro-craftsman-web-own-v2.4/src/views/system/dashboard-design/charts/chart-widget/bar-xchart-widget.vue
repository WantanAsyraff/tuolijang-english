...
export default {
  ...
  mounted() {
    this.cutField = this.field
    this.initOption()
    EventBus.$on('barXChangeValue', this.handleBarXChangeValue)
  },
  destroyed() {
    EventBus.$off('barXChangeValue', this.handleBarXChangeValue)
  },
  methods: {
    handleBarXChangeValue(e, type) {
      if (type) {
        this.drillDown = true
      }
    ...
    async getChartData(options, type, isClick) {
      ...
      let res = await queryChartData(options, type,chartData)
