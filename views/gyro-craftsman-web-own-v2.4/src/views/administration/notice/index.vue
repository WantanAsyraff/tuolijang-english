                  ...
                  <el-table-column prop="name" :label="$('ui.administrationNoticeIndexCover')" min-width="80">
                    <template slot-scope="scope">
                      <el-image
                        ...
                        fit="cover"
                        :preview-src-list="scope.row.cover ? [scope.row.cover] : undefined"
                        class="table-img"
                      />
                    </template>
                  </el-table-column>
...
export default {
  ...
  computed: {
    hasAddPermission() {
      return this.$store.getters.permissions.includes('administration:notice:add')
    }
  },
  mounted() {
    this.getTargetCate()
