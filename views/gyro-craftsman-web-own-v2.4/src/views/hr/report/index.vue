...
const defaultTimeRange = [moment().startOf('month').format('YYYY/MM/DD'), moment().format('YYYY/MM/DD')]
...
export default {
  ...
  data() {
    return {
      ...
      totalPage: 0,
      // 时间选择器的值
      timeVal: defaultTimeRange,
      // 表格查询参数
      tableFrom: this.getDefaultTableFrom(),
      // 部门列表
      departmentList: [],
      // 搜索表单配置
  ...
  methods: {
    ...
    getDefaultSearch() {
      return [
        ...
        },
        {
          ...
          data_dict: [...defaultTimeRange],
          value: defaultTimeRange.join('-')
        }
      ]
    },
    setOptions() {
    ...
    confirmData(data) {
      if (data == 'reset') {
        ...
        this.tableFrom = { ...this.tableFrom, ...data, page: 1 }
        this.getList(1)
      }
    },
    restFn() {
      this.tableFrom = this.getDefaultTableFrom()
      this.timeVal = [...defaultTimeRange]
