...
<div>
  ...
  <!-- 添加群发 -->
  <addGroupPosting v-if="isShow" :types="1" :editId="editId" @backFn="backFn"></addGroupPosting>
  <!-- 详情 -->
...
export default {
  ...
  methods: {
    handleCheck(val) {
      this.$refs.massDetails.openBox(val.id, '1')
    },
    backFn() {
      this.editId = 0
      this.isShow = false
    ...
    async handleRemind(row) {
      await this.$modalSure('confirm.remindGroupOwnerMassSend')
    ...
    addDataFn() {
      this.isShow = true
      // this.$router.push({ path: `${roterPre}/customer/weChatMass/addGroupPosting`, query: { types: '1' } })
    },
    handleEdit(item) {
      this.editId = item.id
      this.isShow = true
