...
<script>
import { getWorkReplyGroupCreateApi, getWorkReplyGroupEditApi, getWorkReplyGroupDelApi } from '@/api/weCom'
export default {
  name: 'DepartmentNot',
  props: {
    leftList: {
  ...
  methods: {
    ...
    addCategory() {
      this.$modalForm(getWorkReplyGroupCreateApi()).then(({ message }) => {
        this.$emit('getTargetCate')
      })
    },
    addDivsion(id) {
      this.$modalForm(getWorkReplyGroupEditApi(id)).then(({ message }) => {
        this.$emit('getTargetCate')
      })
    },
    async handleDelete(id) {
      await this.$modalSure('你确定要删除该分类吗')
      const res = await getWorkReplyGroupDelApi(id)
      if (res.status === 200) {
        this.$emit('getTargetCate')
