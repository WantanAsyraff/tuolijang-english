...
<div>
  <el-form ref="contractForm" label-width="auto" class="contract-form">
    ...
    <!-- 签约信息 -->
    <div class="form-section">
      <div class="section-title">{{ $t("ui.customerSigningInfoItemSigningInformation") }}</div>
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item :label="$t('ui.customerSigningInfoItemContractName')">
      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item :label="$t('ui.customerSigningInfoItemSigningMethod')">
            <span class="info-text">{{ dataInfo.sign_type == 2 ? $t('ui.customerSigningInfoItemESign') : $t('ui.customerSigningInfoItemOfflineSigning') }}</span>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item :label="$t('ui.customerSigningInfoItemElectronicContract')">
        ...
        <el-col :span="12">
          <el-form-item :label="$t('ui.userCalendarAddTodoStartTime')">
            <span class="info-text">{{ dataInfo.start_date || '--' }}</span>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item :label="$t('ui.userCalendarAddTodoEndTime')">
            <span class="info-text">{{ dataInfo.end_date || '--' }}</span>
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item :label="$t('ui.customerSigningInfoItemSigner')">
            <div v-for="(item, index) in dataInfo.signatory" :key="index">
              <div class="signer-info" v-if="item.types == 0">
                <span class="status" v-if="item.sign_status == 1">{{ $t("ui.customerSigningInfoItemSigned") }}</span>
                <span class="status" v-if="item.sign_status == 2">{{ $t("ui.userExamineExamineRejected") }}</span>
                <span class="status" v-if="item.sign_status == 0">{{ $t("ui.customerSigningInfoItemPendingSigning") }}</span>
                <div class="signer-name">
                  {{ item.company_name || '--' }}
                  <span class="company-icon">{{ $t("ui.customerSigningInfoItemOurCompany") }}</span>
                </div>
                <div class="signer-name mt6">
                  <span class="mr25">{{ $t("ui.customerSigningInfoItemHandler") }}{{ item.name || '--' }}</span>
                  <span>{{ $t("ui.customerSigningInfoItemPhoneNumber") }}{{ item.phone || '--' }}</span>
                </div>
              </div>
              <div class="signer-info" v-else>
                ...
                <div class="signer-name">
                  ...
                  <span class="company-icon individual" v-if="item.types == 1">{{ $t("ui.commonOaFromBoxPersonal") }}</span>
                  <span class="company-icon individual" v-else>{{ $t("ui.customerSigningInfoItemEnterprise") }}</span>
                </div>
                <div class="signer-name mt6">
                  <span>{{ $t("ui.customerSigningInfoItemPhoneNumber") }}{{ item.phone || '--' }}</span>
                </div>
        ...
        <el-col :span="12">
          <el-form-item :label="$t('ui.customerSigningInfoItemFileBeforeSigning')">
            <oa-uploadList v-if="dataInfo.sign_file.length > 0" :fileList="dataInfo.sign_file"></oa-uploadList>
            <span v-else>--</span>
          </el-form-item>
          <el-form-item :label="$t('ui.customerSigningInfoItemFileAfterSigning')">
            <oa-uploadList
    ...
    <div class="form-section">
      <div class="section-title">{{ $t("ui.customerSigningAddContractSignProductList") }}</div>

      <productList ref="productList" :type="`edit`" :product="dataInfo.products"></productList>
    </div>

    <div class="form-section">
      <div class="section-title">{{ $t("ui.xmindEditorToolbarNodeBtnListRemarks") }}</div>
      <el-row :gutter="20">
        <el-col :span="12">
