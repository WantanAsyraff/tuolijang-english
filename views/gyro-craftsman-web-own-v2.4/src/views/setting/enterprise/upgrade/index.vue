  ...
  <div class="box-height">
    <el-card>
      <div class="header">
        <div>
          <div>
            {{ $("ui.settingEnterpriseUpgradeIndexCurrentVersion") }}<span class="v"></span><span class="num">{{ version || 'Tuoluojiang' }}</span>
          </div>
          <div class="info title mt20">
            {{ $("ui.settingEnterpriseUpgradeIndexReleaseNotes") }}
            <span v-if="!status">{{ $("ui.settingEnterpriseUpgradeIndexTheSystemIsAlreadyUpToDate") }}</span>
            <ul v-if="status">
              <li>V{{ info.first_version }}.{{ info.second_version }}.{{ info.third_version }}</li>
            </ul>
          </div>
        </div>
        <el-button v-if="status" type="primary" class="primary btn update" @click="update()">{{ $("ui.settingEnterpriseUpgradeIndexUpgradeNow") }}</el-button>
      </div>
    </el-card>
    <el-card class="mt20 employees-card">
      <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane :label="$('ui.settingEnterpriseUpgradeIndexSystemUpgrade')" name="first">
          ...
          <div class="defult" v-if="activities.length == 0">
            ...
            <div class="text">{{ $("ui.settingEnterpriseUpgradeIndexNoSystemUpdates") }}</div>
          </div>
        </el-tab-pane>
        <el-tab-pane :label="$('ui.settingEnterpriseUpgradeIndexUpgradeHistory')" name="second">
          ...
          <div class="defult" v-if="logActivities.length == 0">
            <img src="@/assets/images/def2.png" alt="" class="img" />
            <div class="text">{{ $("ui.settingEnterpriseUpgradeIndexNoUpgradeHistory") }}</div>
          </div>
        </el-tab-pane>
      </el-tabs>
    </el-card>
    ...
    <el-dialog :visible.sync="exoneration" width="672px" :show-close="false" :close-on-click-modal="false">
      <div class="dialog1">
        <div class="exoneration-header">{{ $("ui.settingEnterpriseUpgradeIndexDisclaimer") }}</div>
        <div class="dialog1-content" v-html="agreemen.content"></div>
        <div class="footer">
          <div class="btn" @click="refuse">{{ $("ui.settingEnterpriseUpgradeIndexRefuse") }}</div>
          <div class="btn2" @click="agree">{{ $("ui.settingEnterpriseUpgradeIndexAgree") }}</div>
        </div>
      </div>
    </el-dialog>
    <!-- 在线升级 -->
    <el-dialog :visible.sync="upgrade" width="380px" :show-close="false" :close-on-click-modal="false">
      <div class="dialog d-upgrade">
        <div class="img">{{ $("ui.settingEnterpriseUpgradeIndexUpgradeToV") }}{{ infoUpgrade }}</div>
        <div v-if="upgradeProgress.speed != '100.0' || this.downloadStatus != '200'">
          ...
          <div class="flex">
            ...
            <div class="text">{{ $("ui.settingEnterpriseUpgradeIndexUpdatingPleaseWait") }}</div>
          </div>
        </div>

        <template v-if="upgradeProgress.speed == '100.0'">
          <img src="@/assets/images/ok.png" alt="" class="ok-img" />
          <div class="ok-text">{{ $("ui.settingEnterpriseUpgradeIndexUpgradeSuccessful") }}</div>
          <el-button type="primary" round class="ok-btn mt20" @click="back">{{ $("ui.settingEnterpriseUpgradeIndexOk") }}</el-button>
        </template>
      </div>
    </el-dialog>
  </div>
  ...
  methods: {
    ...
    getUpgradeStart() {
      getUpgradeStart(this.info.package_key)
        ...
        .catch((err) => {
          this.$message.error(this.$('legacyScript.downloadStopped'))
          clearInterval(this.timer)
          this.timer = null
          this.upgrade = false
        })
    ...
    back() {
      ...
      logout()
        .then((res) => {
          this.$message.success(this.$('legacyScript.upgradeCompletedSignInAgain'))
    ...
    format() {
      // let text = '升级中！';
      return (this.upgradeProgress.tip ? this.$(this.upgradeProgress.tip) : this.$('legacyScript.upgrading')) + '\n'
    },
