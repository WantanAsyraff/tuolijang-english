...
<div class="divBox">
  ...
  <print-preview ref="printPreview"  :print-data="printData" />
</div>
</template>
<script>
...
export default {
  ...
  components: {
    ...
    printPreview: () => import('./components/printPreview')
  },
  data() {
    return {
      where: {
  ...
  methods: {
    ...
    // 打印
    handlePrint(row) {
      approveApplyEditApi(row.id, { types: 1 }).then((res) => {

        this.printData = res.data

        this.$refs.printPreview.openBox(res.data)
