...
export default {
  ...
  methods: {
    ...
    async getContract(eid) {
      const customerIds = Array.isArray(eid) ? eid.filter(Boolean) : eid ? [eid] : []
      if (!customerIds.length) {
        this.contractList = []
        return
      }
      const result = await selectContractListApi({ data: customerIds })
...
.stationBox ::v-deep .edui-editor-iframeholder {
...
::v-deep .custom-select .el-icon-circle-close {
