...
export const uploader = async (file, uploadType, option) => {
  let fileSize = getStorageJson('sitedata', {}).global_attach_size || 30
  return new Promise(async (resolve, reject) => {
   ...
   // 获取api上传参数:云盘上传不需要调用
    ...
    if (option && !option.uploadRes) {
      ...
      apiRes = await getUploadKeysApi(obj)
    } else {
      apiRes = option.uploadRes
    }
     // 判断是否有自定义上传地址，没有的话就使用默认地址
  if (!option.url&&apiRes.data.type!=='local') {
...
export const baseUpload = {
      ...
      cos.sliceUploadFile(
        ...
        function (err, data) {
          if (err) {
            ...
            resolve({ url: processResourceUrl('http://' + data.Location), ETag: data.ETag })
          }
        }
      )
    })
  ...
  ossHttp(evfile, res, videoIng) {
    ...
    let url = processResourceUrl(data.host)
    ...
    return new Promise((resolve, reject) => {
      ...
      axios
        ...
        .then(() => {
          // that.progress = 0;
          videoIng(false, 0)
          resolve({ url: processResourceUrl(fileUrl) })
        })
        .catch((res) => {
          reject({ msg: res })
        })
  ...
  obsHttp(file, res, videoIng) {
    ...
    const url = processResourceUrl(data.host)
    ...
    return new Promise((resolve, reject) => {
      ...
      axios
        ...
        .then(() => {
          videoIng(false, 0)
          resolve({ url: processResourceUrl(data.cdn ? data.cdn + '/' + filename : fileUrl) })
        })
        .catch((res) => {
          reject({ msg: res })
        })
  ...
  us3Http(file, res, videoIng) {
    ...
    return new Promise((resolve, reject) => {
      ...
      axios
        ...
        .then((res) => {
          videoIng(false, 0)
          resolve({ url: processResourceUrl(data.cdn ? data.cdn + '/' + filename : url) })
        })
        .catch((res) => {
          reject({ msg: res })
        })
  ...
  qiniuHttp(evfile, res, videoIng, option) {
    ...
    const filename = new Date().getTime() + suffix
    const fileUrl = processResourceUrl(res.data.domain) + '/' + filename
    const config = {
      useCdnDomain: true
    }
    const putExtra = {
    ...
    return new Promise((resolve, reject) => {
      observable.subscribe({
        ...
        complete: (result) => {
          // 接收成功后返回的信息
          videoIng(false, 0)
          resolve({ url: processResourceUrl(res.data.cdn ? res.data.cdn + '/' + filename : fileUrl) })
        }
      })
    })
  },
