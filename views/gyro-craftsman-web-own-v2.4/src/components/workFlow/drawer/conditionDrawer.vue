...
export default {
  ...
  watch: {
    ...
    conditions1(val) {
      if(val.length>0){
        val = val.filter(item => typeof item === 'object')
      }
  let type = ['input','approvalBill','uploadFrom']
      this.newConfig = val.filter((item) => !type.includes(item.type))
      this.conditions = val.filter((item) => !type.includes(item.type))
    },
