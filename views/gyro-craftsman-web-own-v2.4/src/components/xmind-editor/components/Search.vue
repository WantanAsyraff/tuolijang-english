<template>
  <div class="searchContainer" :class="{ show: show }">
...
export default {
  ...
  data() {
    return {
      ...
      listenEventMap: {
        ...
        [SEARCH_INFO_CHANGE]: this.handleSearchInfoChange,
        [NODE_CLICK]: this.blur,
        [DRAW_CLICK]: this.blur,
        [EXPAND_BTN_CLICK]: this.blur,
        [SEARCH_MATCH_NODE_LIST_CHANGE]: this.onSearchMatchNodeListChange
      }
    }
  },
  watch: {
  ...
  created() {
    ...
    Object.entries(this.listenEventMap).forEach(([eventName, handler]) => {
