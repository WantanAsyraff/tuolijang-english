...
<div class="mlset-conditions" v-loading="loading">
  <div class="mlconditions-list-box">
    <el-row class="mb-10 mlconditions-list" :gutter="10" v-for="(item, inx) of conditionConf.items" :key="inx">
      <!-- 字段名 -->
      <el-col :span="10">
        <div class="field-one">
...
export default {
  ...
  data() {
    return {
      ...
      op_type: {},
      // 不需要输入框的条件
      op_no_value: ['NL', 'NT', 'SFU', 'SFB', 'SFD', 'YTA', 'TDA', 'TTA', 'CUW', 'CUM', 'CUQ', 'CUY'],
      // 数组类型的字段
  ...
  methods: {
    ...mapMutations(['queryEntityNameByCode']),
    openReferenceDialog() {
    ...
    getSelectOp(item) {
      ...
      if (type == 'Reference') {
        ...
        op = this.conditionsConfig[type] ? [...this.conditionsConfig[type]] : []
      }
      return op
    },
    // 添加条件
