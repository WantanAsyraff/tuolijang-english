<!-- @FileDescription: 文件预览弹窗-mp3/mp4/png -->
<template>
  <div>
    <!-- mp3 -->
    <el-dialog
    ...
    >
      ...
      <div class="mp4-box">
        <mp4 v-if="is_mp4Show" :url="url"></mp4>
      </div>
    </el-dialog>
    <!-- 图片 -->
  ...
  methods: {
    ...
    openFile(fid, id, data) {
      if (fid) {
        ...
        this.file = data
        this.file.file_ext = data.name.split('.').pop()
        if (this.file.file_ext == 'mp3') {
          this.is_mp3Show = true
        } else if (this.file.file_ext == 'mp4') {
          this.is_mp4Show = true
        } else {
          this.srcList.unshift(data.url)
          this.$refs.imageViewer.openImageViewer(data.url)
        }
        this.url = data.url
      }
    }
