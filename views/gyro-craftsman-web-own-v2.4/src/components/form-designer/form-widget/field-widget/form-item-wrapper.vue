  ...
  <div class="field-wrapper" :class="{ 'design-time-bottom-margin': !!this.designer }">
    <el-form-item
      ...
      :class="[
        ...
        labelAlign,
        customClass,
        field.options.required ? 'required' : 'not-required'
      ]"
...
export default {
  ...
  methods: {
    ...
    removeFieldWidget() {
      if (!!this.parentList) {
        // 保存要删除的字段名称
        const fieldRefName = this.field.options.name
...
.not-required ::v-deep .el-form-item__label::before {
  content: '' !important;
  display: none;
}
