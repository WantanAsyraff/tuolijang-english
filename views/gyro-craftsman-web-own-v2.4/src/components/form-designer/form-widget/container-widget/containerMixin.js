...
export default {
  ...
  methods: {
    ...
    onContainerDragAdd(evt, subList,type) {
    
      const newIndex = evt.newIndex
      let addedItem = subList[newIndex]
    ...
    removeWidget(removeWidget) {
      if (!!this.parentList) {
        ...
        this.$nextTick(() => {
          ...
          if (removeWidget.cols && removeWidget.cols.length > 0) {
            removeWidget.cols.map((item) => {
              this.designer.formWidget.deleteWidgetRef('', item)
            })
          }
          if (removeWidget.widgetList && removeWidget.widgetList.length > 0) {
            this.designer.formWidget.deleteWidgetRef(widgetRefName, removeWidget) //删除组件ref！！！
          }
          this.designer.emitHistoryChange()
        })
      }
