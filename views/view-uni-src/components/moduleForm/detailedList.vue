<template>
  <view>
    <view v-for="(item, index) in tableData" :key="index">
      <view class="title">
        <view
          >{{ $t('ui.moduleFormDetailedListDetails') }} <text v-if="tableData.length > 1">{{ index + 1 }}</text></view
        >
      ...
      <view class="addText" @click="addFn">{{ $t('ui.moduleFormDetailedListAddDetails') }}</view>
    </view>
  </view>
</template>
