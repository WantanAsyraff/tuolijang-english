<template>
  <view class="content">
          ...
          <view v-for="(val, index) in listData" :key="index">
            ...
            <uni-forms-item class="input-label" required name="name" v-if="val.form_value === 'input'">
              ...
              <uni-easyinput
                ...
                :autoHeight="true"
                :maxlength="val.max"
                :placeholder="String($ts($t('ui.examineFormTimeFromPleaseEnter') + val.field_name))"
              ></uni-easyinput>
            </uni-forms-item>
            ...
            <uni-forms-item class="is-direction-top" v-if="val.form_value === 'rich_text'">
              ...
              {{ $ts(val.options.placeholder) }}
              <c-editor :content="formData[val.field_name_en]" :placeholder="$t('ui.attendanceExternalWorkModalEnterContent')" @saveContent="saveContent($event, val)"></c-editor>
            </uni-forms-item>
            ...
            <uni-forms-item class="input-label" v-if="val.form_value === 'checkbox'">
              ...
              >
                {{ ($t('ui.examineFormCustomCheckboxPleaseSelect') + val.field_name) }}
                <view class="iconfont icon-fanhui"></view>
              </view>
              <view v-else class="picker-input label-flex" @click="changeCheck(val, index)">
                <text>{{ $ts(val.text) }}</text>
              </view>
            ...
            <uni-forms-item class="input-label" v-if="val.form_value == 'cascader'">
              ...
              <view v-else class="picker-input label-flex" @click="changeCascde(val, index)">
                <text class="overflow-x">{{ $ts(val.text) }}</text>
                <view class="iconfont icon-fanhui"></view>
              </view>
            </uni-forms-item>
            ...
            <uni-forms-item class="input-label" v-if="val.form_value === 'radio'">
              ...
              <picker mode="selector" :value="val.value" :range="$localize(val.data_dict)" range-key="name" @change="pickerChange($event, val, index)">
                <view v-if="!formData[val.field_name_en]" class="picker-input picker-input-placeholder">
                  {{ ($t('ui.examineFormCustomCheckboxPleaseSelect') + val.field_name) }}
                  <view class="iconfont icon-fanhui"></view>
                </view>
            ...
            <uni-forms-item class="input-label" v-if="val.form_value === 'cascader_address'">
              ...
              <uni-data-picker
                :localdata="$localize(addressData)"
                :map="{ text: 'name', value: 'value' }"
                :popup-title="String($ts(val.placeholder))"
            ...
            <uni-forms-item class="input-label" v-if="val.form_value === 'cascader_radio'">
              ...
              <uni-data-picker
                :localdata="$localize(val.data_dict)"
                :popup-title="String($ts(val.placeholder))"
                :map="{ text: 'name', value: 'value' }"
            ...
            <uni-forms-item class="is-direction-top" v-if="val.form_value === 'image'">
              <template v-slot:label>
                <view class="uni-forms-item__label mt36 p24">
                  {{ $ts(val.field_name) }}
                  <text class="tips"> {{ $t('ui.moduleFormIndexRecommended7341034Max2MbJpgJpegAnd') }} </text>
                </view>
                ...
                <view class="upload">
                  ...
                  >
                    ...
                    <view class="text">{{ $t('ui.financeBillAddUploadImage') }}</view>
                  </view>
                </view>
              </template>
            </uni-forms-item>
    ...
    <cascade
      ref="cascdeRef"
      :title="$t('ui.moduleFormIndexSelectCategory')"
