...
import { ajCaptchaCheckApi, ajCaptchaDataApi } from "@/api/public";
export default {
  name: "VerifySlide",
  props: {
    captchaType: {
  ...
  methods: {
    ...
    // 鼠标松开
      ...
      if (this.status && this.isEnd == false) {
        ...
        ajCaptchaCheckApi(data).then((result) => {
          let res = result.data;
    ...
    getPictrue() {
      ...
      ajCaptchaDataApi(data).then((result) => {
        let res = result.data;
