<template>
		...
		<template v-if="contractData.length > 0">
			<view class="contract-info-item" v-for="(item, index) in contractData" :key="index"
				...
				<view class="contract-header">
					...
					<view v-if="item.status" class="status-badge" :style="{
						...
						background: statusList[item.status].color
							...
							: getColor('#1890ff', '0.1')
					}">{{ $ts(statusList[item.status].name) }}</view>
				</view>
				<view class="contract-body">
					...
					<uni-row class="contract-item" style="margin-bottom: 8px;">
						<uni-col :span="5" class="left">{{ $t('ui.customerSigningDetailItemContractAmount') }}</uni-col>
					...
					<uni-row class="contract-item" style="margin-bottom: 8px;">
						<uni-col :span="5" class="left">{{ $t('ui.customerListCustomerContractStartAndEndTime') }}</uni-col>
	...
	<view class="footer-text" v-if="contractData.length > 0 && count == contractData.length">{{ $t('ui.customerListFollowRecordNoMore') }}</view>
	</view>
