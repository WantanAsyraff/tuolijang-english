        ...
        <view class="mb130">
          ...
          <view class="per">
            <view class="per-title">
              {{ $t('ui.customerListStatisticsPerformanceBrief') }}
              <view class="line" />
            </view>
          ...
          <view class="per">
            <view class="per-title">
              <text> {{ $t('ui.customerListStatisticsProductCategoryStatistics') }}</text>
              <view class="line" />
            </view>
            ...
            <view class="charts-box" v-if="data.contractList2.length > 0">
              <qiun-data-charts type="ring" :opts="$localize(data.opts2)" :chartData="$localize(chartData2)" @getIndex="tap2" />
            </view>
            <view v-else class="no-data">{{ $t('ui.customerListStatisticsNoData') }}</view>
          </view>
          <!-- 订单分类统计 -->
          <view v-if="data.contractCategoryEnabled" class="per">
            <view class="per-title">
              <text> {{ $t('ui.customerListStatisticsOrderCategoryStatistics') }}</text>
              <view class="line" />
            </view>
            <view class="el-breadcrumb">
            ...
            <view class="charts-box" v-if="data.contractList.length > 0">
              <qiun-data-charts type="ring" :opts="$localize(data.opts)" :chartData="$localize(chartData)" @getIndex="tap" />
            </view>
            <view v-else class="no-data">{{ $t('ui.customerListStatisticsNoData') }}</view>
          </view>
          ...
          <view class="per" v-if="data.salesmanList.length > 0">
            <view class="per-title">
              {{ $t('ui.customerListStatisticsSalesPerformanceRanking') }}
              <view class="line" />
            </view>
            <view class="per-ranking">
              <view class="title">
                <text>{{ $t('ui.customerListStatisticsPerformanceRanking') }}</text>
                <text>{{ $t('ui.customerListStatisticsPerformanceAmountCny') }}</text>
              </view>
              <view class="list">
          ...
          <view class="per product-per" v-if="data.productRankList.length > 0">
            <view class="per-title">
              {{ $t('ui.customerListStatisticsProductPerformanceRanking') }}
              <view class="line" />
            </view>
            <view class="per-ranking">
              <view class="title">
                <text>{{ $t('ui.customerListStatisticsRanking') }}</text>
                <text class="w160">{{ $t('ui.customerListStatisticsProduct') }}</text>
                <text class="mr10">{{ $t('ui.customerListStatisticsSalesVolume') }}</text>
                <text>{{ $t('ui.customerListStatisticsNetPerformance') }}</text>
              </view>
              <view class="list">
                <view class="list-item" v-for="(item, index) in data.productRankList" :key="index">
                  <view class="left w200">
                    ...
                    <view class="overText"> [{{ item.spec_type == 1 ? $t('ui.customerListStatisticsMultipleSpecs') : $t('ui.customerListStatisticsSingleSpec') }}] {{ item.product_name }} </view>
                  </view>
