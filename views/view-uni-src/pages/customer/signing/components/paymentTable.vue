<template>
  <view class="examine-content-list">
        ...
        <template v-slot:body>
          <view class="item-list">
            <view @click="examineList(item)">
              ...
              <uni-row class="item-list-content">
                <uni-col :span="5" class="left">{{ $t('ui.customerSigningOrderListPaymentStatus') }}</uni-col>
                <uni-col :span="19">{{ item.payment_status === 1 ? $t('ui.customerSigningOrderListSettled') : $t('ui.customerSigningOrderListUnsettled')}}</uni-col>
              </uni-row>
              <uni-row class="item-list-content">
    ...
    <empty v-else :index="7" :title="$t('ui.customerContractContractListNoContractData')"></empty>
    <view class="fixed-btn" @click="handleClick">
     <text class="iconfont icon-kaoqin-beizhu1" /> {{ $t('ui.customerSigningPaymentTableLinkOrder') }}
    </view>
  </view>
</template>
