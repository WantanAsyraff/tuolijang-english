  ...
  <view class="content">
        ...
        <view class="details-content">
          <view class="company-name line1" v-if="detail.status"
            ...
            <text
              ...
              >{{ $ts(statusList[detail.status].name) }}</text
            >
          </view>
          <view class="info-content"> {{ $t('ui.customerContractDetailsCustomerName') }}{{ detail.customer && detail.customer.customer_name ? detail.customer.customer_name : '--' }} </view>
          <view class="info-content">
            {{ $t('ui.customerListCustomerMoreSalesperson') }}{{ detail.admin && detail.admin.name ? detail.admin.name : '--' }}
            <text class="line" />
            {{ $t('ui.customerSigningDetailsStartAndEndTime') }}
    ...
    <view class="customer">
      ...
      <view v-if="data.currentId === 3">
        ...
        <view class="footer-text" v-if="data.orderList.length > 0 && data.count <= data.orderList.length">{{ $t('ui.customerListFollowRecordNoMore') }}</view>
      </view>
    </view>
    <!-- 操作弹窗 -->
