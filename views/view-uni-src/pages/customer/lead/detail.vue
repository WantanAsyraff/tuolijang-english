<template>
  ...
  <view class="content">
          ...
          <view class="company-header">
            <view @click.stop="openCustomerChat(leadInfo.work_customer)" class="display-align">
              ...
              <text :class="leadInfo.work_customer?.type != 1 ? 'work-name1' : 'work-name'" v-if="leadInfo.work_customer?.type">
                {{ leadInfo.work_customer.type == 1 ? $t('ui.customerLeadLeadListWeChat') : '@' + leadInfo.work_customer.corp_name || '--' }}
              </text>
            ...
            <view
              ...
              >{{ $ts(leadInfo.status.name) }}
            </view>
          </view>

          <view class="customer-info">
            {{ $t('ui.customerListCustomerMoreSalesperson') }}{{ salesInfo || '--' }}
            <text class="divider"></text>
            {{ $t('ui.customerLeadDetailAddTime') }} {{ moment(leadInfo?.created_at).format('YYYY/MM/DD HH:mm:ss') }}
          </view>
    ...
    <view class="details-fixed-btn" style="width: 361rpx">
      <view @click="bottomBtnListFn(1)" class="box"><text class="iconfont icon-zhuankehu"></text>{{ $t('ui.customerLeadDetailConvertToCustomer') }}</view>
      <view class="btn-line" />
      <view @click="bottomBtnListFn(2)" class="box"><text class="iconfont icon-dadianhua" />{{ $t('ui.customerListDetailsFootCall') }}</view>
    </view>
    <!-- 操作弹窗 -->
    <more-popup ref="customerMoreRef" @handleItem="dropDownItem"></more-popup>
    <selected-label :title="$t('ui.customerListCustomerMoreCustomerLabels')" ref="selectedLabelRef" @changeItem="changeItem" />
    <!-- 备注弹窗 -->
