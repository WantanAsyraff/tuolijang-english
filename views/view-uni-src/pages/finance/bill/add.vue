  ...
  <BaseContainer>
    ...
    <view class="bill-add-body">
      <view class="bill-add-card">
        <view class="bill-card-title">{{ $t('ui.customerEditFormBasicInformation') }}</view>
        <view class="bill-card-item">
          <view class="bill-card-item-label">{{ $t('ui.financeBillAddAccountType') }}</view>
        ...
        <view class="bill-card-item">
          <view class="bill-card-item-label">{{ $t('ui.financeBillAddAccountCategory') }}</view>
          <view class="bill-card-item-value">
        ...
        <view class="bill-card-item">
          <view class="bill-card-item-label">{{ $t('ui.financeBillAddAccountAmount') }}</view>
          <view class="bill-card-item-value">
            <input :placeholder="$t('ui.attendanceShiftAddEnter')" class="input-content" placeholder-class="input-placeholder" type="digit"
              v-model="billDetailData.form.num" />
          </view>
        </view>

        <view class="bill-card-item">
          <view class="bill-card-item-label">{{ $t('ui.customerContractPayDetailPaymentMethod') }}</view>
          <view class="bill-card-item-value">
        ...
        <view class="bill-card-item">
          <view class="bill-card-item-label">{{ $t('ui.financeBillAddPaymentTime') }}</view>
          <view class="bill-card-item-value">
            <uni-datetime-picker type="datetime" :end="maxDateRange" :border="false"
              ...
              <view class="picker-content" :class="getCssByValue(billDetailData.form.editTime)">
                {{ billDetailData.form.editTime || $t('ui.examineFormCustomCheckboxPleaseSelect') }}
                <text class="iconfont icon-jinru-copy" />
              </view>
            </uni-datetime-picker>
          </view>
      ...
      <view class="bill-add-card remark-card">
        <view class="bill-card-title">{{ $t('ui.customerSigningDetailItemRemarkInformation') }}</view>
        <textarea v-model="billDetailData.form.mark" :maxlength="-1" class="bill-card-text-area"
          :placeholder="$t('ui.financeBillAddPleaseEnterTheRemarkInformation')"></textarea>
      </view>

      <view class="bill-add-card">
        <view class="bill-card-title">{{ $t('ui.financeBillAddEntryAttachments') }}</view>
        <view class="bill-card-subtitle">
          {{ $t('ui.financeBillAddUploadImage') }}
          <text>{{ $t('ui.financeBillAdd1ImageJpgJpegOrPng') }}</text>
        </view>
    ...
    <view class="bill-submit-btn-wrapper">
      <button @click="handleSaveBillDetails" class="bill-submit-btn" :loading="saveLoading"
        :disabled="saveLoading">{{ $t('ui.replyComponentIndexSubmit') }}</button>
    </view>
  </BaseContainer>
</template>
