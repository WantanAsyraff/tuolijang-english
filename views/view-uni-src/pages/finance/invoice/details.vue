<template>
  <view class="content">
    ...
    <view class="cr-examine-button" v-if="data.detail.status === 5||data.detail.status === 1">
      <button class="button default-error" v-if="data.detail.status === 5" @click="toExamine(2)">{{ $t('ui.financeInvoiceDetailsInvalidate') }}</button>
      <button v-if="data.detail.status === 1" class="button agree" @click="toExamine(1)">{{ $t('ui.financeInvoiceDetailsEnterInvoice') }}</button>
    </view>
