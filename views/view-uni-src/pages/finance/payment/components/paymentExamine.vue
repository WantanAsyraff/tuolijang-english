  ...
  <uni-popup ref="popupRef" type="right" :mask-click="false">
    <view class="slider">
      ...
      <view class="slider-tips plr10" v-if="configData.type !== 0"><text class="iconfont icon-yemiantishi"></text>
        {{configData.type === 2 ? $t('ui.financePaymentPaymentExamineAfterSavingTheIncomeExpenseEntryAndTheRelated') : $t('ui.financePaymentPaymentExamineAfterApprovalTheSystemWillAutomaticallyGenerateAnIncome')}}
      </view>
      <view class="content">
          ...
          <view class="m10">
            <uni-forms :border="false" label-width="80px">
              <view class="form-item-list" v-if="configData.type !== 0">
                ...
                <uni-forms-item class="input-label" v-if="configData.type !== 2">
                  <template v-slot:label>
                    <view class="uni-forms-item__label">{{ $t('ui.financeBillAddAccountCategory') }} <text class="iconfont">*</text></view>
                  </template>
                  <view class="picker-tree-data">
                    <uni-data-picker class="tree-data" :placeholder="$t('ui.customerListAddSpendPleaseSelectAccountCategory')" :popup-title="$t('ui.financeBillAddAccountCategory')"
                ...
                <uni-forms-item class="input-label">
                  <template v-slot:label>
                    <view class="uni-forms-item__label">{{getPayRecordTypes(configData.row.types)}}{{ $t('ui.financePaymentPaymentExamineAmountYuan') }} <text
                        class="iconfont">*</text></view>
                  </template>
                  <uni-easyinput v-model="formData.num" :inputBorder="false" type="number" :clearable="false"
                    maxlength="11" :autoHeight="true" :placeholder="$t('ui.attendanceShiftAddEnter')">
                  </uni-easyinput>
                </uni-forms-item>
                ...
                <uni-forms-item class="input-label">
                  <template v-slot:label>
                    <view class="uni-forms-item__label">{{configData.type < 2 ? $t('ui.financePaymentPaymentExaminePayment') : $t('ui.financePaymentPaymentExamineExpense')}}{{ $t('ui.financePaymentPaymentExamineMethod') }}<text
                        class="iconfont">*</text></view>
                  </template>
                ...
                <uni-forms-item class="input-label">
                  <template v-slot:label>
                    <view class="uni-forms-item__label">{{configData.type < 2 ? $t('ui.financePaymentPaymentExaminePayment2') : $t('ui.financePaymentPaymentExamineExpense')}}{{ $t('ui.financePaymentPaymentExamineTime') }}<text
                        class="iconfont">*</text></view>
                  </template>
                ...
                <uni-forms-item class="input-label is-direction-top">
                  <template v-slot:label>
                    <view class="uni-forms-item__label item-label-left">{{configData.type < 2 ? $t('ui.financePaymentPaymentExaminePayment2') : $t('ui.financePaymentPaymentExamineExpense')}}{{ $t('ui.financePaymentPaymentExamineReceipt') }}</view>
                    <view class="upload">
                    ...
                    <text class="tips">{{ $t('ui.customerContractAddPaymentRecommended7341034Max') }}{{fileSizeOne}}{{ $t('ui.financeInvoiceInvoiceExamineMbJpgJpegPngAndOtherFormatsSupported') }}</text>
                  </template>
                </uni-forms-item>
              </view>
              ...
              <view class="form-item-list">
                <uni-forms-item class="input-label is-direction-top">
                  <template v-slot:label>
                    <view class="uni-forms-item__label item-label-left">{{configData.type !== 0 ? $t('ui.customerContractPayDetailRemarks') : $t('ui.financeInvoiceInvoiceExamineReasonForRejection')}}</view>
                  </template>
                  <uni-easyinput :inputBorder="false" v-model="formData.mark" type="textarea" :clearable="false"
                    :maxlength="256" :autoHeight="true" :placeholder="configData.type !== 0 ? $t('ui.financeInvoiceInvoiceExaminePleaseEnterRemarks') : $t('ui.financeInvoiceInvoiceExamineEnterTheReasonForRejection')">
                  </uni-easyinput>
                </uni-forms-item>
              </view>
            </uni-forms>
      ...
      <view class="examine-button">
        <button type="primary" class="button" :loading="loading"
          @click="handleConfirm">{{configData.type === 2 ? $t('ui.attendanceShiftAddSave') : $t('ui.replyComponentIndexSubmit')}}</button>
      </view>
    </view>
  </uni-popup>
</template>
