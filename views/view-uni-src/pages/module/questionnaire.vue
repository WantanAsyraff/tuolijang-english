<template>
  <BaseContainer class="custom-container" v-if="formInfo.unique">
    ...
    <template v-if="formInfo.designInfo.length">
      ...
      <button class="submit-btn" @click="handleSubmit">{{ $t('ui.replyComponentIndexSubmit') }}</button>
    </template>
    <view class="success-mask" v-if="formInfo.submitSuccess">
      ...
      <view class="success-text">{{ $t('ui.moduleQuestionnaireYourQuestionnaireHasBeenSubmittedThankYouForParticipating') }}</view>
    </view>
  </BaseContainer>
</template>
