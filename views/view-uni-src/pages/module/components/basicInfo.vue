<template>
  <view class="box">
    <view class="title" v-if="!type">{{ $t('ui.customerEditFormBasicInformation') }}</view>
      ...
      <template v-if="val.form_value !== 'details'">
        ...
        <uni-col :span="19" class="info-item-right">
          ...
          <view v-else-if="val.form_value === 'switch'">
            <view>{{ val.value == 1 ? $t('ui.attendanceSignInButtonYes') : $t('ui.attendanceSignInButtonNo') }}</view>
          </view>
          ...
          <text v-else>
            {{ ['cascader_address', 'cascader_radio', 'radio', 'checkbox'].includes(val.form_value) ? $ts(getValue(val)) : (getValue(val) || '--') }}
          </text>
        </uni-col>
      </template>
      <uni-col :span="24">
