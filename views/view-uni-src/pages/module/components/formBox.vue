  ...
  <view class="content">
    ...
    <view class="picker-selector" v-for="(item, index) in info.seniorSearch" :key="index">
      ...
      <uni-data-picker v-if="['cascader_address'].includes(item.form_value)" v-model="data.where[item.field_name_en]"
        :localdata="$localize(data.addressData)" :map="{ text: 'name', value: 'value' }" @change="cityChange($event, item, index)">
      ...
      <!----级联单选--->
      <uni-data-picker v-if="['cascader_radio'].includes(item.form_value)" :localdata="$localize(item.data_dict)"
      ...
      <!----开关---->
      <picker mode="selector" v-if="item.form_value == 'switch'" :value="item.value" :range="$localize(data.switchList)"
      ...
      <!----单选---->
      <picker mode="selector" v-if="['radio'].includes(item.form_value)" :value="item.value" :range="$localize(item.data_dict)"
      ...
      <view class="search-default-label" @click="changeCheck(item, index)" v-if="item.form_value == 'checkbox'">
        ...
        <text v-if="data.where[item.field_name_en]" class="iconfont date-clear icon-shenpizhongxin-jujue"
          @click.stop="clickClear(item)"></text>
        <multiplePicker :show="data.show" :columns="$localize(item.data_dict)" @change="multiplePickerChange($event, item)"
          @cancel="ontouchcancel"></multiplePicker>
      </view>
    </view>
  </view>
  <selected-label :title="$t('ui.moduleFormBoxSelectLabel')" ref="selectedLabelRef" :listData="data.labelData" @changeItem="changeItem"
    @resetLabel="resetLabel"></selected-label>
  <!-- 级联复选组件 -->
  <cascade ref="cascdeRef" :title="$t('ui.moduleFormIndexSelectCategory')" :listData="data.cascadeData" @reset="resetCascade" @change="changeCascade"
