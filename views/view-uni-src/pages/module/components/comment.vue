<template>
  <view class="comment-box">
    <view class="title">
      {{info.comment_title||$t('ui.moduleCommentComments')}}
    </view>
    <view class="content">
              ...
              <uni-row class="right-top">
                ...
                <uni-col :span="12" class="text-right">
                  <view v-if="userInfo.id == item.user.id">
                    <text @click="deleteReply(item)">{{ $t('ui.examineFormApprovalBillDelete') }}</text>
                  </view>
                  <view v-else>
                    <text @click="examineReply(item)">{{ $t('ui.moduleCommentEvaluate') }}</text>
                  </view>
                </uni-col>
              </uni-row>
                  ...
                  <view class="reply-item">
                    ...
                    <view class="reply-right">
                      <view class="flex-between">
                        ...
                        <view class="text-right" v-if="userInfo.id == val.user.id" @click="deleteReply(val)">{{ $t('ui.examineFormApprovalBillDelete') }}</view>
                        <view class="text-right" v-else @click="examineReply(val)">{{ $t('ui.moduleCommentEvaluate') }}</view>
                      </view>
    ...
    <view class="replay">
      <uni-row class="display-align" v-if="data.examineReplyBtn">
        <uni-col :span="16" class="replay-left">
          <textarea maxlength="50" auto-height :focus="data.examineReplyBtn" @confirm="changeInput"
            v-model="data.content" :placeholder="$t('ui.moduleCommentYouCanLeaveAComment')" />
        </uni-col>
      ...
      <view v-else class="examine-bottom-reply" @click="data.examineReplyBtn=true">
        ...
        <view class="name"> {{info.comment_title||$t('ui.moduleCommentComments')}} </view>
      </view>
    </view>
  </view>
</template>
