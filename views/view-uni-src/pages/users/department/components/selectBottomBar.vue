      ...
      <view class="plr10">
        <uni-row>
          <uni-col :span="18">
            <view class="select-content-left">
              <view class="top" @click="handleCheckPer">
                {{ $t('ui.attendanceShiftSelected') }} {{ selectPersonnel.length }} {{showPerson ? $t('ui.attendanceSchedulePeople') : $t('ui.usersDepartmentSelectBottomBarDepartment')}}
          ...
          <uni-col :span="6" class="select-content-right text-center">
            <button type="primary" size="mini" @click="clickOk">{{ $t('ui.baTreePickerIndexOk') }}</button>
          </uni-col>
        </uni-row>
      </view>
    </view>
