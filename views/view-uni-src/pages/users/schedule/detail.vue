<template>
  <view class="content" v-if="loading">
    <view class="cr-position-header default-header">
      ...
      <view class="nav-bar">
        <default-nav-bar :index="1"  :default-title="$t('ui.usersReportMineDetails')" :is-right="isRight"
    ...
    <view class="report-con m10" :class="!isRight ? 'pbc' : ''">
      <view class="report-list-item pb28">
        <view class="title">{{ $t('ui.usersScheduleCreateParticipants') }}</view>
      ...
      <view class="report-list-item" style="padding-top: 0;padding-bottom: 0;">
        <uni-row class="report-list-item-bottom">
          <uni-col class="bottom-left" :span="24">{{ $t('ui.usersScheduleCreateTodoContent') }}</uni-col>
      ...
      <view class="report-list-item" style="padding-top: 0;padding-bottom: 0;">
        <uni-row class="report-list-item-bottom">
          <uni-col class="bottom-left" :span="5">{{ $t('ui.usersScheduleCreateScheduleType') }}</uni-col>
          <uni-col :span="19">{{data.info.type.name}}</uni-col>
        </uni-row>
        <uni-row class="report-list-item-bottom ">
          <uni-col class="bottom-left" :span="5">{{ $t('ui.customerListAddFollowReminderTime') }}</uni-col>
          <uni-col :span="19"
      ...
      <view class="btn-box">
        ...
        <view v-if="!isRight" class="flex-box">
          ...
          <view v-for="(item ,index) in bottomBtn" :key="index" class="mr19 flex-box" @click="clickButton(item)">
            ...
            <text class="button" :class="data.selectInfo&&data.selectInfo.status === item.id ? 'active' : 'no-active'"
              ...
              {{data.info.finish === item.id  ? $t('ui.usersScheduleDetailAlready') : '' }}{{item.text}}
            </text>
          </view>
        </view>
        <view v-else class="flex-box">
          ...
          <view class="button" :style="{ '--fill-color': data.info.finish ==3?'#308BF8':'#606266'}"
            @click="putStatus('完成', 3)">{{data.info.finish == 3 ? $t('ui.usersScheduleDetailCompleted') : $t('ui.examineFormTimeFromPickerCompleted') }}</view>
        </view>
