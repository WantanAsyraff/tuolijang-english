<template>
  <view class="content">
    ...
    <view class="schedule m10" :style="{ '--height': cHeight + 'px' }">
            ...
            <view class="mb20" :id="val.date">
              ...
              <view v-if="val.list.length == 0" class="add-box">
                ...
                <view class="no-schedule"
                  >{{ $t('ui.usersScheduleScheduleNoSchedules') }}
                  <template v-if="moment(val.date).isSameOrAfter(moment().format('YYYY-MM-DD'))"
                    ><text> ,</text> <text class="add-text" @click="clickCreate(val)">{{ $t('ui.usersScheduleScheduleClickToCreate') }}</text></template
                  >
                </view>
              </view>
      ...
      <empty v-if="data.listData.length <= 0 || (!complete && !data.allFinish)" :index="3" :title="$t('ui.usersScheduleScheduleNoToDoTasks')"></empty>
    </view>
    <!-- 新增 -->
    <view class="add">
