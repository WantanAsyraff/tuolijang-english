<template xmlns="">
  <view class="content">
    ...
    <view v-if="serverConfigInfo.length > 1" class="enterprise" @click="goUrl('/pages/users/login/enterprise')">
      ...
      <text>{{ $t('ui.usersLoginIndexEnterprises') }}</text>
    </view>
    <view class="content-login p-l-r30">
      <image class="logo" :src="chooseEnterprise?.logo || '../../../static/image/logo.png'"></image>
      <view class="title">{{ chooseEnterprise?.enterprise_name || $t('ui.usersLoginIndexSignInToTuolujiangEnterpriseAssistant') }}</view>
      ...
      <view class="form">
        <uni-forms ref="forms" :modelValue="formData" label-position="top">
          <template v-if="index === 0">
            <uni-forms-item :label="$t('ui.userUserPhonePhoneNumber')">
              <uni-easyinput v-model="formData.phone" :maxlength="11" :clearable="false" @input="inputPhone"
                :adjust-position="false" :styles="{ borderColor: 'transparent' }" :placeholder="$t('ui.forumLoginPopPleaseEnterYourMobilePhoneNumber')" />
            </uni-forms-item>
            <uni-forms-item class="forms-item-text" :label="$t('ui.usersCenterIndexPassword')">
              ...
              <text class="default-color item-text" @click="handleForget">{{ $t('ui.forgotPasswordIndexForgetPassword') }}</text>
            </uni-forms-item>
          </template>
          <template v-if="index === 1">
            <uni-forms-item :label="$t('ui.userUserPhonePhoneNumber')">
              <uni-easyinput v-model="formData.phone" :clearable="false" :maxlength="11" :adjust-position="false"
                @input="inputPhone" :styles="{ borderColor: 'transparent' }" :placeholder="$t('ui.forumLoginPopPleaseEnterYourMobilePhoneNumber')" />
            </uni-forms-item>
            <uni-forms-item class="forms-item-text" :label="$t('ui.forgotPasswordIndexCode')">
              <uni-easyinput v-model="formData.verificationCode" :maxlength="6" :clearable="false"
                :adjust-position="false" :styles="{ borderColor: 'transparent' }" :placeholder="$t('ui.forumLoginPopPleaseEnterTheCorrectVerificationCode')" />
      ...
      <view class="display-align agree-content">
        <checkbox-group @change="changeAgree">
          <label>
            ...
            {{ $t('ui.usersLoginIndexIHaveReadAndAgreeTo') }}
          </label>
        </checkbox-group>
        <navigator url="/pages/users/login/service" animation-type="slide-in-right" class="default-color">{{ $t('ui.usersLoginIndexTermsOfService') }}
        </navigator>
        {{ $t('ui.usersLoginIndexAnd') }}
        <navigator url="/pages/users/login/privacy" animation-type="slide-in-right" class="default-color">{{ $t('ui.usersLoginIndexPrivacyPolicy') }}
        </navigator>
      </view>
      <button class="preserve" type="primary" :loading="loading" :disabled="handleVerButton()" @click="handlePreserve">{{ $t('ui.usersLoginIndexLogin') }}</button>
      <view class="login-text">
        {{ index ? $t('ui.usersLoginIndexANewAccountWillBeCreatedAutomaticallyWhenSigning') : $t('ui.usersLoginIndexNewUsersShouldSignInWithSms') }}
      </view>
    </view>
