  ...
  <view class="content">
      ...
      <view class="assess-bottom">
        <uni-row class="display-align">
          <uni-col :span="data.rowSpan">
            ...
            <view class="text-center capitin" @click="explainClick">{{ $t('ui.usersAssessmentDefaultTotalScore') }} <text v-if="data.explain" class="iconfont icon-bangzhu"></text></view>
          </uni-col>
          ...
          <uni-col :span="data.rowSpan">
            ...
            <view class="text-center capitin">{{ $t('ui.usersAssessmentDefaultSelfScoringSelfEvaluation') }}</view>
          </uni-col>
          ...
          <uni-col :span="data.rowSpan" v-if="data.assessInfo.status > 1">
            ...
            <view class="text-center capitin"> {{ data.assessInfo.status > 3 ? $t('ui.usersAssessmentDefaultPerformanceScore') : $t('ui.usersAssessmentDefaultCurrentScore') }}</view>
          </uni-col>
        </uni-row>
      </view>
    </view>
            ...
            <view class="assessment-header">
              <uni-row>
                ...
                <uni-col :span="12" class="assessment-header-right line1">
                  {{ data.assessInfo.types === 0 ? $t('ui.usersAssessmentDefaultWeight') : $t('ui.usersAssessmentDefaultScore') }}：{{ item.ratio }}
                  {{ data.assessInfo.types === 0 ? '%' : '' }}
                  {{ $t('ui.usersAssessmentDefaultScore2') }}{{ currentSum(item, index) }}
                </uni-col>
              </uni-row>
            </view>
                  ...
                  <template v-slot:body>
                    <view class="assessment-list-item">
                      ...
                      <uni-forms :border="false" label-position="top" label-width="80px">
                        <uni-forms-item :label="$t('ui.usersAssessmentDefaultIndicatorDescription')">
                          <uni-easyinput
                            :inputBorder="false"
                            :disabled="true"
                            type="textarea"
                            ...
                            :placeholder="$t('ui.usersAssessmentDefaultPleaseEnterIndicatorDescription')"
                          >
                          </uni-easyinput>
                        </uni-forms-item>
                        <uni-forms-item :label="$t('ui.usersAssessmentDefaultRatingLevel')">
                          <uni-easyinput
                            :inputBorder="false"
                            ...
                            :autoHeight="true"
                            v-model="items.info"
                            :placeholder="$t('ui.usersAssessmentDefaultPleaseEnterTheRatingLevel')"
                          >
                          </uni-easyinput>
                        </uni-forms-item>
                        <uni-forms-item :label="$t('ui.usersAssessmentDefaultCompletionStatus')">
                          <uni-easyinput
                            :inputBorder="false"
                            :disabled="data.assessInfo.status === 5"
                            type="textarea"
                            ...
                            :placeholder="$t('ui.usersAssessmentDefaultPleaseEnterTheCompletionStatus')"
                          >
                          </uni-easyinput>
                        </uni-forms-item>
                        <uni-forms-item>
                          <template v-slot:label>
                            <uni-row class="list-item-label display-align">
                              <uni-col :span="12">
                                <view>{{ data.assessInfo.types === 0 ? $t('ui.usersAssessmentDefaultCompletionSelfAssessment') : $t('ui.usersAssessmentDefaultSelfRating') }}</view>
                              </uni-col>
                              <uni-col :span="12" class="text-right">
                                <view
                                  >{{ data.assessInfo.types === 0 ? $t('ui.usersAssessmentDefaultWeight') : $t('ui.usersAssessmentDefaultMaximumIndicatorScore') }}：{{ items.ratio }}
                                  {{ data.assessInfo.types === 0 ? '%' : '' }}
                                </view>
                              </uni-col>
                            </uni-row>
                        ...
                        <uni-forms-item v-if="data.assessInfo.status > 2" class="check-items">
                          <template v-slot:label>
                            <uni-row class="list-item-label display-align">
                              <uni-col :span="12">
                                <view>{{ $t('ui.usersAssessmentDefaultTheHigherEvaluation') }}</view>
                              </uni-col>
                              <uni-col :span="12" class="text-right">
                                <view>{{ $t('ui.usersAssessmentDefaultTheSuperiorGrade') }}{{ items.score }} {{ data.assessInfo.types === 0 ? '%' : '' }}</view>
                              </uni-col>
                            </uni-row>
                          </template>
                          <uni-easyinput
                            ...
                            :maxlength="512"
                            :autoHeight="true"
                            :placeholder="$t('ui.usersAssessmentDefaultNoEvaluation')"
                          >
                          </uni-easyinput>
                        </uni-forms-item>
                      </uni-forms>
    ...
    <view class="assessment-button display-align">
      <template v-if="current > 1 && current !== data.info.length">
        <button class="assess-btn upper mr10" plain="true" @click="clickUpper">{{ $t('ui.usersAssessmentDefaultPrevious') }}</button>
        <button class="assess-btn ml10" type="primary" @click="clickDown">{{ $t('ui.usersAssessmentDefaultNext') }}</button>
      </template>
      <template v-else-if="current == 1 && data.info.length === 1">
        <button :disabled="data.assessInfo.status === 5" class="assess-btn" type="primary" :loading="loading" @click="clickSubmit">{{ $t('ui.examineFormTimeFromPickerCompleted') }}</button>
      </template>
      <template v-else-if="current > 1 && current === data.info.length">
        <button class="assess-btn upper mr10" plain="true" @click="clickUpper">{{ $t('ui.usersAssessmentDefaultPrevious') }}</button>
        <button :disabled="data.assessInfo.status === 5" class="assess-btn ml10" type="primary" :loading="loading" @click="clickSubmit">{{ $t('ui.examineFormTimeFromPickerCompleted') }}</button>
      </template>
      <template v-else>
        <button class="assess-btn" type="primary" @click="clickDown">{{ $t('ui.usersAssessmentDefaultNext') }}</button>
      </template>
    </view>
