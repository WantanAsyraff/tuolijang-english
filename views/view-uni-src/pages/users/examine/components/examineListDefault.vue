        ...
        <template v-slot:body>
          <view class="item-list">
            <view class="item-list-top" @tap="examineList(item)" v-if="item.card">
              <text>
                {{ item.card.name }}{{ $t('ui.indexIndexS') }}{{ item.approve.name }}
              </text>
            ...
            <view class="item-list-content item-list-button">
              <template v-if="typeIndex ==-1">
                <button class="default-error" @click="getApplyRevoke(item)" style="color:#303133;"
                  v-if="((item.status === 1 && item.rules && item.rules.recall == 1) || item.status === 0) && !item.recall">{{ $t('ui.usersExamineExamineListDefaultRevoke') }}</button>
                <button class="default-color" @click="resubmit(item)"
                  v-if="item.status === 1 && item.type < 6">{{ $t('ui.usersExamineExamineListDefaultResubmit') }}</button>
              </template>
              <template v-else>
                <template v-if="item.status !== 1">
                  <button v-if="item.verify_status === 0" class="default-error"
                    @tap="handleRefuse(item, index)">{{ $t('ui.financePaymentDetailsRefuse') }}</button>
                  <button v-if="item.verify_status === 0" class="default-color"
                    @tap="handleAgree(item, index)">{{ $t('ui.financePaymentDetailsAgree') }}</button>
                </template>
              </template>
            </view>
