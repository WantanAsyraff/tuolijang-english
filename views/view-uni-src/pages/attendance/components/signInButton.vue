<template>
  <view>
    ...
    <view v-if="showAddress" class="address">
      ...
      <text class="upload" @click="emit('refreshLocation')">{{ $t('ui.attendanceSignInButtonRefresh') }}</text>
    </view>
    ...
    <view v-if="showWifiInfo" class="wifi-info">
      ...
      <view class="wifi-bssid">
        <view @click="emit('copyWifiMac')">{{ $t('ui.attendanceSignInButtonMacAddress') }} {{ wifiBssid }}</view>
        <view class="refresh" @click="handleRefreshWifi">{{ $t('ui.attendanceSignInButtonRefresh') }}</view>
      </view>
      <view>{{ $t('ui.attendanceSignInButtonWithinWiFiRange') }} {{ isWifiRange ? $t('ui.attendanceSignInButtonYes') : $t('ui.attendanceSignInButtonNo') }}</view>
    </view>
    <!-- #endif -->
  </view>
</template>
