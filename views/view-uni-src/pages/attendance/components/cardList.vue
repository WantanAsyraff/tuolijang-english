  ...
  <view class="nav">
    <view class="card">
        ...
        <view class="item" v-for="(item, index) in list" :key="index">
          ...
          <view class="msg">
            <text
              ...
              :key="i"
              >{{ ex.type == 1 ? $t('ui.attendanceCardListStartWork') : $t('ui.attendanceCardListFinishWork')
              }}{{ ex.location_status == 1 ? $t('ui.attendanceCardListNormal') : $t('ui.attendanceCardListException')
              }}{{ i / 2 == 0 ? "，" : "" }}</text
            >
          </view>
        </view>
	  ...
	  <view v-else class="default">
	  	...
	  	<view class="text">{{ $t('ui.attendanceCardListNoOffSiteClockStatistics') }}</view>
	  </view>
    </view>
  </view>
</template>
