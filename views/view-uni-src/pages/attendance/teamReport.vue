  ...
  <view class="nav" v-if="!loading">
    <uni-nav-bar :fixed="true" left-icon="left" status-bar backgroundColor="rgba(119,176,247,1)" color="#fff"
      ...
      <view class="nav-content">
        <view class="title">{{ $t('ui.attendanceTeamAttendanceTeamMonthlyReport') }}</view>
      </view>
      <template #right>
    ...
    <view class="card pb48">
      <view class="header" @click="jump(3)">
        <view class="title">
          {{ $t('ui.attendancePersonalReportOvertimeStatistics') }}
          <text class="gard">{{ $t('ui.attendanceTeamReportPeople') }}</text>
        </view>
        <view class="jump">
          <text class="iconfont icon-jinru-copy"></text>
        </view>
      ...
      <view class="statistics schedule">
        <view class="sta-data">
          <view class="sta-data-item" @click="overTimeFn(1,'工作日加班')">
            ...
            <view class="text">{{ $t('ui.attendancePersonalReportWeekdayOvertime') }}</view>
          </view>
          <view class="sta-data-item" @click="overTimeFn(2,'休息日加班')">
            <view class="num">{{ overtimeStatistics.rest }}</view>
            <view class="text">{{ $t('ui.attendancePersonalReportRestDayOvertime') }}</view>
          </view>
          <view class="sta-data-item" @click="overTimeFn(3,'节假日加班')">
            <view class="num">{{ overtimeStatistics.holiday }}</view>
            <view class="text">{{ $t('ui.attendancePersonalReportHolidayOvertime') }}</view>
          </view>
        </view>
      </view>
    </view>
    <view class="card pb48">
      <view class="header" @click="jump(2)">
        <view class="title">
          {{ $t('ui.attendancePersonalReportLeaveStatistics') }}
          <text class="gard">{{ $t('ui.attendanceTeamReportPeople') }}</text>
        </view>
        <view class="jump">
          <text class="iconfont icon-jinru-copy"></text>
