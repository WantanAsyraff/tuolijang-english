  ...
  <view class="nav" v-if="!loading">
          ...
          <picker ref="picker" mode="date" fields="month" :value="monthTime" start="2021-01" :end="lastTime"
            ...
            <view class="dis-flex">
              <text class="uni-nav-bar-text">
                <text class="month">{{ monthTime.split('-')[1] }}{{ $t('ui.examineFormTimeFromPickerMonth') }}</text>
              </text>
    ...
    <view class="card">
      ...
      <view class="schedule">
        <view class="sta">
          ...
          <view class="tip">{{ $t('ui.attendancePersonalReportNormalDaysExceptionDays') }}</view>
        </view>
        ...
        <view class="sta-data">
          <view class="sta-data-item" @click="goDetails(1, '地点异常')">
            ...
            <view class="text">{{ $t('ui.attendancePersonalReportLocationExceptionsTimes') }}</view>
          </view>
          ...
          <view class="sta-data-item" @click="goDetails(6, '旷工')">
            ...
            <view class="text">{{ $t('ui.attendancePersonalReportAbsenceDays') }}</view>
          </view>
        </view>
      </view>
    </view>
    <view class="card">
      <view class="header" @click="jump(1)">
        <view class="title"> {{ $t('ui.attendancePersonalReportOvertimeStatistics') }} </view>
        <view class="jump">
          <text class="iconfont icon-jinru-copy"></text>
        </view>
      </view>
      <view class="table-list">
        <view class="item" @click="overTimeFn(0, '工作日加班')">
          <view class="num">{{ $t('ui.attendancePersonalReportWeekdayOvertime') }}</view>
          <view class="msg">{{ overtimeStatistics.work }}{{ $t('ui.examineFormTimeFromHours') }}</view>
        </view>
        <view class="item" @click="overTimeFn(2, '休息日加班')">
          <view class="num">{{ $t('ui.attendancePersonalReportRestDayOvertime') }}</view>
          <view class="msg">{{ overtimeStatistics.rest }}{{ $t('ui.examineFormTimeFromHours') }}</view>
        </view>
        <view class="item" @click="overTimeFn(1, '节假日加班')">
          <view class="num">{{ $t('ui.attendancePersonalReportHolidayOvertime') }}</view>
          <view class="msg">{{ overtimeStatistics.holiday }}{{ $t('ui.examineFormTimeFromHours') }}</view>
        </view>
      </view>
    </view>
    <view class="card">
      <view class="header" @click="jump(2)">
        <view class="title"> {{ $t('ui.attendancePersonalReportLeaveStatistics') }} </view>
        <view class="jump">
          <text class="iconfont icon-jinru-copy"></text>
        </view>
      </view>
      <view class="table-list">
        <view class="item" v-for="(item, index) in leaveStatistics" :key="index"
          ...
          <view class="msg">{{ item.num }}{{ $t('ui.attendanceRulesSecond') }}</view>
        </view>
      </view>
    </view>
  </view>
