<template>
  <view class="main">
    <view class="header-fixed">
      <uni-nav-bar background-color="transparent" :border="false" status-bar left-icon="left" :title="$t('ui.attendanceScheduleScheduleManagement')" dark
        class="custom-nav-bar" right-icon="plusempty" @clickRight="handleAdd" />
      <view class="search-bar">
        <view class="search-bar-body">
          <view class="iconfont icon-sousuo"></view>
          <input type="text" :placeholder="$t('ui.attendanceScheduleEnterAttendanceGroupName')" placeholder-class="placeholder" @confirm="handleSearch" />
        </view>
      </view>
    ...
    <view class="schedule-list">
      <navigator class="schedule-item" v-for="item of scheduleData.list" :key="item.id"
        ...
        hover-class="none">
        <view class="schedule-title">{{ formatScheduleMonth(item.date) }}{{ $t('ui.attendanceScheduleSchedule') }}</view>
        <view class="schedule-info">
          <view class="schuedling-info-label">{{ $t('ui.attendanceScheduleAttendanceGroup') }}</view>
          <view class="schuedling-info-value">{{ item.group.name }}</view>
        </view>
        <view class="schedule-info">
          <view class="schuedling-info-label">{{ $t('ui.attendanceScheduleAttendanceMembers') }}</view>
          <view class="schuedling-info-value flex">
            ...
            <view class="member-count">{{ $t('ui.attendanceScheduleEtc') }}{{ item.group.members.length }}{{ $t('ui.attendanceSchedulePeople') }}</view>
          </view>
        </view>
      </navigator>
    </view>
    <view class="empty-container" v-if="scheduleData.list.length === 0">
      ...
      <view class="empty-text">{{ $t('ui.attendanceScheduleNoScheduleInformation') }}</view>
    </view>
