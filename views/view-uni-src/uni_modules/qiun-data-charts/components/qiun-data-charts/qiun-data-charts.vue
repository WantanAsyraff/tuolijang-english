    ...
    methods: {
      beforeInit() {
        ...
        if (typeof this.chartData === 'object' && this.chartData != null && this.chartData.series !== undefined && this
          ...
          this.mixinDatacomLoading = false;
          // 如果这里直接进行赋值，则 canvas 无法获取到容器正确的宽高
          // this.showchart = true; 
...
<script module="rdcharts" lang="renderjs">
  ...
  import cfu from '../../js_sdk/u-charts/config-ucharts.js';
  import cfe from '../../js_sdk/u-charts/config-echarts.js';
  import { registerEchartsLiquidFillPlugin } from '../../static/echarts-liquidfill.js';
    ...
    methods: {
      ...
      ecinit(newVal, oldVal, owner, instance) {
        ...
        if (typeof window.echarts === 'object') {
          ...
          /**
           * 在 uniapp 中使用 echarts 时，由于 uniapp 会向全局注入 wx.* 相关方法
           * 导致 echarts 环境识别出现问题，无法在移动端触发点击事件
           * 需要在 echarts 初始化之前暂时将 wx 环境变量屏蔽掉
           * refer： https://github.com/ecomfe/zrender/issues/1024
           * 
           * 由于同一页面可能存在多个 echarts 实例，该部分会被执行多次，所以只在最后一个 echarts 实例加载完成时恢复原始变量
           */
          
          if (typeof uni !== "undefined" && typeof wx !== "undefined") {
            if (wx.getSystemInfoSync) {
              uni.__global_wx_origin_getSystemInfoSync = wx.getSystemInfoSync;
            }
            wx.getSystemInfoSync = null;
            if (uni.__global_wx_env_count === undefined) {
              uni.__global_wx_env_count = 1;
            } else {
              uni.__global_wx_env_count++;
            }
          }
          /**
           * end
          ...
          script.onload = () => {
            this.newEChart();
            /**
             * echarts 脚本加载完成后，恢复相关变量 
             */
            if (uni.__global_wx_origin_getSystemInfoSync) {
                uni.__global_wx_env_count--;
                if (uni.__global_wx_env_count === 0) {
                  wx.getSystemInfoSync = uni.__global_wx_origin_getSystemInfoSync;
                  uni.__global_wx_origin_getSystemInfoSync = undefined;
                }
            }
            /**
             * end
      ...
      newEChart() {
        registerEchartsLiquidFillPlugin(echarts);
        let cid = this.rid
        if (!cid) return;
        if (cfe.instance[cid] === undefined) {
          const initOpts = {};

          if (cfe.option[cid].containerWidth && cfe.option[cid].containerHeight) {
            const { containerHeight, containerWidth } = cfe.option[cid];
            initOpts.width = containerWidth;
            initOpts.height = containerHeight;
          }
          
          cfe.instance[cid] = echarts.init(that[cid].$el.children[0], null, initOpts)
          //ontap开启后才触发click事件
      ...
      updataEChart(cid, option) {
        ...
        if (
          ...
          /**
           * 已经手动传入容器大小，无需再 resize，否则会打断加载动画。
           */
          // this.ecresize();
         }
      },
      ...
      touchMove(e) {
        ...
        if (cfu.option[cid].ontap === true && cfu.option[cid].enableScroll === false && cfu.option[cid].onmovetip ===
          ...
          let tmpe = {
            ...
            y: e.changedTouches[0].clientY - rchartdom.top +
              rootdom.top
          }
          e.changedTouches.unshift(tmpe)
