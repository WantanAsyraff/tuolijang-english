...
function getFunnelDataPoints(series, radius, option, eachSpacing) {
  ...
  for (let i = 0; i < series.length; i++) {
    if(option.type == 'funnel'){
      ...
      series[i].radius = (series[i].data + 3) / series[0].data * radius * process;
      /**
       * end
       */
    }else{
...
function getXAxisPoints(categories, opts, config) {
   ...
   * 增加 X 轴垂直网格线外边距配置
  */
  const { splitNumberMarginLeft, splitNumberMarginRight } = opts.xAxis;
  if (splitNumberMarginLeft) spacingValid -= splitNumberMarginLeft;
  if (splitNumberMarginRight) spacingValid -= splitNumberMarginRight;
   ...
   * 调整 X 轴垂直网格线渲染起始位置
   */
  const xAxisStartX = splitNumberMarginLeft ? splitNumberMarginLeft + startX : startX;
  categories.forEach(function(item, index) {
    xAxisPoints.push(xAxisStartX + widthRatio / 2 * eachSpacing + index * eachSpacing);
  });
  /**
   * end
   */
...
function getBarDataPoints(data, minRange, maxRange, yAxisPoints, eachSpacing, opts, config) {
  ...
  var validWidth = opts.width - opts.area[1] - opts.area[3];
    /**
   * 根据 x 轴绘制偏移量，减少条形柱的高度
   */
  const { splitNumberMarginLeft, splitNumberMarginRight } = opts.xAxis;
  if (splitNumberMarginLeft) validWidth -= splitNumberMarginLeft;
  if (splitNumberMarginRight) validWidth -= splitNumberMarginRight;
  /**
   * end
   */
  data.forEach(function(item, index) {
    if (item === null) {
       ...
       * 增加 x 轴绘制偏移量
       */
      if (splitNumberMarginLeft) {
        point.x += splitNumberMarginLeft;
      }
      /**
       * end
       */
...
function getBarStackDataPoints(data, minRange, maxRange, yAxisPoints, eachSpacing, opts, config, seriesIndex, stackSeries) {
   ...
   */
  const { splitNumberMarginLeft, splitNumberMarginRight } = opts.xAxis;
  if (splitNumberMarginLeft) validHeight -= splitNumberMarginLeft;
  if (splitNumberMarginRight) validHeight -= splitNumberMarginRight;
  /**
   * end
   */
  data.forEach(function(item, index) {
    if (item === null) {
       ...
       */
      if (splitNumberMarginLeft) {
        point.x0 += splitNumberMarginLeft;
      }
      /**
       * end
       */
...
function drawPieText(series, opts, config, context, radius, center) {
   ...
   */
    ...
    const labelValueShow = opts?.extra?.pie?.labelValueShow;
    const labelValueColor = opts?.extra?.pie?.labelValueColor;

    const labelText1 = labelValueShow ? series[i].name : item.text;
    const fontSize = item.textSize * opts.pix || config.fontSize;

    context.beginPath();
    context.setFontSize(fontSize);
    context.setFillStyle(item.textColor || opts.fontColor);
    context.fillText(labelText1, textStartX, textPosition.y + 3);
    context.closePath();

    if (labelValueShow) {
      context.beginPath();
      context.setFontSize(fontSize);
      context.setFillStyle(labelValueColor || item.textColor || opts.fontColor);
      context.fillText(item.text, textStartX, textPosition.y + fontSize + 8);
      context.closePath();
    }
...
function drawToolTip(textList, offset, opts, config, context, eachSpacing, xAxisPoints) {
  ...
  /**
 * start
 * 这一段是增加对 tooltip 阴影的支持
 */

  if (toolTipOption.boxShadow) {
    const { shadowOffsetX, shadowOffsetY, shadowColor, shadowBlur } = toolTipOption.boxShadow;
    context.shadowOffsetY = shadowOffsetX;
    context.shadowOffsetX = shadowOffsetY;
    context.shadowColor = shadowColor;
    context.shadowBlur = shadowBlur;
    context.fill()
    context.shadowColor = "rgba(0,0,0,0)"; // 清除阴影设置，防止后续对文字的影响
  }
      ...
      if (item.color !== null) {
        ...
        switch (item.legendShape) {
          ...
          case 'hollowCircle':
            ...
            context.moveTo(startX + 7.5 * opts.pix, startY + 0.5 * legendWidth);
            context.arc(startX + 7.5 * opts.pix, startY + 0.5 * legendWidth, 3 * opts.pix, 0, 2 * Math.PI);
            // debugger;
            break;
          /**
           * end
  ...
  textList.forEach(function(item, index) {
    ...
    /**
     * 开启分类显示后，第一行靠左对齐，不留图标的空间
     * 支持标题独立设置颜色
     */
    if (index === 0 && toolTipOption?.showCategory) {
      startX -= legendMarginRight + legendWidth;
      context.setFillStyle(toolTipOption.categoryColor || toolTipOption.fontColor);
      // debugger
    } else {
        ...
        for (let i = 0; i < points.length; i++) {
          ...
          if (item !== null && i > leftNum && i < rightNum) {
            ...
            if ((columnOption.barBorderRadius && columnOption.barBorderRadius.length === 4) || columnOption.barBorderCircle === true) {
              ...
              */
              let left = startX;
              const { splitNumberMarginLeft } = opts.xAxis;
              if (splitNumberMarginLeft) {
                left += splitNumberMarginLeft;
              }
              /**
               * end
               */
...
function drawXAxis(categories, opts, config, context) {
  ...
  if (opts.xAxis.axisLine) {
     ...
     * 增加绘制 X 轴线往左侧的偏移量
     */
    const { axisLineOffsetLeft } = opts.xAxis;
    if (axisLineOffsetLeft) startX -= axisLineOffsetLeft;
  ...
  legendList.forEach(function(itemList, listIndex) {
    ...
    for (let i = 0; i < itemList.length; i++) {
      ...
      switch (item.legendShape) {
        /**
         * 增加圆角矩形的绘制
         * uniapp App端不支持 roundRect API，需要手动绘制
         */
        case 'roundedRect':
          const requireOffsetTypes = [
            "ring",
            "radar"
          ];

          // 需要对偏移量进行调整的类型
          const requireOffset = requireOffsetTypes.includes(opts.type);
          const radarOffsetRate = 1.7;
          if (requireOffset) startY = startY * radarOffsetRate;  

          const { itemWidth = 15, itemHeight = 10, itemBorderRadius = 2 } = opts.legend;
          const width = itemWidth * opts.pix;
          const height = itemHeight * opts.pix;
          const radius = itemBorderRadius * opts.pix;
          context.moveTo(startX + radius, startY);
          context.arcTo(startX + width, startY, startX + width, startY + height, radius);
          context.arcTo(startX + width, startY + height, startX, startY + height, radius);
          context.arcTo(startX, startY + height, startX, startY, radius);
...
function drawRadarDataPoints(series, opts, config, context) {
  ...
  var _loop = function _loop(i) {
     ...
     */
    // context.setLineWidth(1 * opts.pix);
    context.setLineWidth((i === radarOption.gridCount ? 2 : 1) * opts.pix);
    /**
     * end
     */
    context.setStrokeStyle(radarOption.gridColor);
...
function drawFunnelDataPoints(series, opts, config, context) {
  ...
  let eachSpacing = (opts.height - opts.area[0] - opts.area[2] - 20) / series.length;
  let centerPosition = {
    x: opts.area[3] + (opts.width - opts.area[1] - opts.area[3]) / 3,
    y: opts.height - opts.area[2] - 10 // Y轴向上进行偏移
  };
...
function drawFunnelText(series, opts, context, eachSpacing, labelAlign, activeWidth, centerPosition) {
  for (let i = 0; i < series.length; i++) {
    ...
    if (labelAlign == 'right') {
       ...
       * 单独渲染 label value
       */
      const textColor = item.textColor || opts.fontColor;
      if (item.labelTips) {
        context.beginPath();
        context.setFontSize(fontSize);
        context.setFillStyle(textColor);
        context.fillText(item.labelTips, endX + 10, startY + fontSize / 2 - 2);
        context.closePath();
        endX = endX + 10 + measureText(item.labelTips, fontSize, context);
      }
      /**
       * end
       */
      ...
      /**
       * 单独渲染 label value时，需要调整字体颜色
       */

      // context.setFillStyle(item.textColor || opts.fontColor);
      // context.fillText(text, endX + 5, startY + fontSize / 2 - 2);
      const labelValueColor = opts?.extra?.funnel?.labelValueColor;
      context.setFillStyle(item.labelTips && labelValueColor ? labelValueColor : textColor);
      context.fillText(text, endX + 8, startY + fontSize / 2 - 2);
      /**
       * end
       */
...
function drawCharts(type, opts, config, context) {
  ...
  var _calLegendData = calLegendData(seriesMA, opts, config, opts.chartData, context),
    /**
     *  增加 10px，增加与 legend 的距离, 原生 legend 不支持单独配置 margin
     */
    legendHeight = _calLegendData.area.wholeHeight + 10,
    /**
     * end
     */
