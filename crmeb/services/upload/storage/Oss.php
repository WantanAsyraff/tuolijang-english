...
class Oss extends BaseUpload
{
     ...
     */
    public function getMate(string $key)
    {
        try {
            ...
            return $this->app()->getObjectMeta($storageName ?: $this->storageName, $fileKey);
        } catch (\Exception $e) {
    ...
    public function stream($fileContent, ?string $key = null)
    {
        try {
            ...
            $this->fileInfo->uploadInfo    = $uploadInfo;
            $this->fileInfo->realName      = basename($key);
            $this->fileInfo->fileSize      = $uploadInfo['info']['size_upload'] ?? 0;
            $this->fileInfo->fileType      = strtolower(pathinfo($key, PATHINFO_EXTENSION));
            $this->fileInfo->filePath      = ($this->cdn ?: $this->uploadUrl) . '/' . $key;
            $this->fileInfo->fileName      = basename($key);
            $this->fileInfo->filePathWater = $this->water($this->fileInfo->filePath);
