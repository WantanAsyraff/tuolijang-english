<template>
  <view class="content" v-if="data.listData">
      ...
      <view class="position examine-info plr10">
        <uni-row class="examine-info-row">
          ...
          <uni-col :span="18" class="examine-info-right">
            <view class="name">
              {{ data.listData.card ? data.listData.card.name : '--' }}的{{ data.listData.approve ? data.listData.approve.name : '--' }}
              <text
                class="status-tag"
                :style="{
                ...
                >{{ statusList[data.listData.status].name }}</text
              >
            </view>
            <view class="time"> 申请时间：{{ moment(data.listData.created_at).format('YYYY/MM/DD HH:mm') }} </view>
          </uni-col>
        </uni-row>
      </view>
    </view>
    ...
    <view class="examine-content" @click="changeInput">
      <view class="steps-content mb10">
        ...
        <view v-if="data.listData.apply_id" class="revoke" @click="revokeFn(data.listData.apply_id)">
          查看需要撤销的申请单 <text class="iconfont icon-jinru-copy"></text>
        </view>
      </view>
      ...
      <view class="replay-con">
        <uni-list :border="false">
          <view class="replay-title" :class="{ 'replay-pb': data.replayData.length == 0 }">留言（{{ data.replayData.length }}）</view>
    ...
    <view class="examine-bottom" v-if="data.typeIndex != -1">
      <view class="footer-box">
        ...
        <template v-if="data.typeIndex != -1 && data.listData.status !== 1 && data.listData.verify_status == 0">
          <button type="warn" plain="true" @click="handleRefuse()"><text class="iconfont icon-shenpizhongxin-jujue"></text>拒绝</button>
          <button type="primary" plain="true" @click="handleAgree()"><text class="iconfont icon-shenpizhongxin-tongyi"></text>同意</button>
        </template>
      </view>
    </view>
    ...
    <view class="details-fixed-btn" v-if="data.typeIndex == -1">
      ...
      <view class="box" v-if="data.listData.approve.types < 6" @click="resubmit()"><text class="iconfont icon-zaicitijiao" />再次提交</view>
      <view class="btn-line" v-if="data.listData.approve.types < 6" />
      <template v-if="![2, -1].includes(data.listData.status)">
        <view class="box" @click="handleRevoke()"><text class="iconfont icon-chexiao" />撤销</view>
      </template>
    </view>
    <global-index></global-index>
    <replyComponent ref="replyComponentRef" :title="`留言`" @submit="submit"></replyComponent>
  </view>
</template>
