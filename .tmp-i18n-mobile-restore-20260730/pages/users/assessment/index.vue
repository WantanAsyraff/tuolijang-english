          ...
          <template v-slot:body>
            <view class="item-list">
              ...
              <view class="item-list-content">
                <view class="items display-align">被考核人 <text >{{item.test.name}}</text></view>
                ...
                <view class="items display-align">考核周期 <text>{{ getPeriodText(item.period) }}</text></view>
              ...
              <uni-row class="item-list-content item-list-button"  v-if="item.status === 1">
                <button class="default-color">进行{{getStatusText(item.status)}}</button>
              </uni-row>
            </view>
          </template>
        </uni-list-item>
      </uni-list>
      <empty v-else :index="2" title="暂无绩效考核内容～"></empty>
    </view>
  </view>
</template>
