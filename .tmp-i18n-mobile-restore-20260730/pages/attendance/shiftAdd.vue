<template>
  <view class="main">
    <view class="header-fixed">
      <uni-nav-bar background-color="transparent" :border="false" status-bar left-icon="left" title="班次设置" dark
  ...
  <view class="page-body">
    <view class="page-card">
      <view class="form-item" style="margin-top: 6rpx">
        <view class="form-item-label" data-required>班次名称</view>
        <input type="text" placeholder="请填写" class="form-item-input" v-model="shiftForm.name" />
      </view>
      <view class="form-item" style="margin-top: 48rpx">
        <view class="form-item-label" data-required>一次上下班</view>
        <PopupListPicker style="flex: 1;" title="上下班次数" :list="commutesNumberConfig"
    ...
    <view class="page-card">
      <view class="card-tips">
        {{ shiftFormIndex.commutesNumberActiveIndex === 0 ? '' : '第1次' }}上班时间设置
      </view>
      <ShiftAddForm @change="handleFormChange" prefix="number1" :form="shiftForm.number1" type="work" />
      <view class="card-tips">
        {{ shiftFormIndex.commutesNumberActiveIndex === 0 ? '' : '第1次' }}下班时间设置
      </view>
    ...
    <view class="overtime-title">加班起算时间</view>
    <view class="page-card">
      <view class="form-item">
        <view class="form-item-label" data-required>最后班次下班后多久开始计算</view>
    ...
    <view class="work-time-total-box">
      <view class="work-time-total-title">
        合计工作时长&nbsp;&nbsp;<text>{{ formatTotalTime(shiftForm.work_time) }}</text>
      </view>
      <view class="work-time-total-btn" @click="handleSave">保存</view>
    </view>
  </view>
</template>
