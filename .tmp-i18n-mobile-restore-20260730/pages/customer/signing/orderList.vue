<template>
  <view class="content">
    ...
    <view class="cr-position-header">
      ...
      <view class="search">
        <view class="required-box"><text>客户名称</text></view>
      ...
      <view class="item" v-for="(item, index) in data.list" :key="index">
        ...
        <view class="list-item">
          ...
          <view class="list-text">
            {{ item.payment_status == 1 ? '已结清' : '未结清' }}
          </view>
        </view>
        <view class="list-item">
          <view class="name"> 订单状态 </view>
          <view class="list-text">{{ item.contract_status && item.contract_status.name ? item.contract_status.name : '--' }}
          </view>
        </view>
        ...
        <empty v-if="data.list.length == 0" :index="7" title="暂无订单数据" ></empty>
