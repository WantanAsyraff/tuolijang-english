    ...
    <view class="examine-content">
      <template v-if="link_type == 5 && isShow">
        <view v-for="(item, index) in oddslist" :key="`odds-${item.id}`" class="item">
          ...
          <view class="list-item">
            <view class="name">商机编号</view>
            <view class="list-text">{{ item.odds_no || '--' }}</view>
          </view>

          <view class="list-item">
            <view class="name">商机类型</view>
            <view class="list-text">{{ item.types?.name || '--' }}</view>
          </view>

          <view class="list-item">
            <view class="name">商机金额</view>
            <view class="list-text">{{ item.total_amount || '--' }}</view>
          </view>
          <view class="list-item">
            <view class="name">商机状态</view>
            <view class="list-text">{{ item.status?.name || '--' }}</view>
          </view>
          <view class="list-item">
            <view class="name">业务员</view>
        ...
        <view v-if="oddslist.length == 0" class="no-data"> 暂无商机数据 </view>
      </template>
      ...
      <template v-if="link_type == 2 && isShow">
        <view v-for="(item, index) in contractList" :key="`contract-${item.id}`" class="item">
          ...
          <view class="list-item">
            <view class="name">付款状态</view>
            <view class="list-text">{{ item.payment_status == 1 ? '已结清' : '未结清' }}</view>
          </view>
          <view class="list-item">
            <view class="name">订单状态</view>
            <view class="list-text">{{ item.contract_status?.name || '--' }}</view>
          </view>
          <view class="list-item">
            <view class="name">业务员</view>
            <view class="list-text">{{ item.salesman?.name || '--' }}</view>
        ...
        <view v-if="contractList.length == 0" class="no-data"> 暂无订单数据 </view>
      </template>
    </view>
