<template>
  <view class="examine-content-list">
        ...
        <template v-slot:body>
          <view class="item-list">
            <view @click="examineList(item)">
              <view class="item-list-top">{{ item.doc_name || '--' }}
                <text class="status-tag" :style="{
                  ...
                  background: statusList[item.status].color
                    ...
                    : getColor('#1890ff', '0.1')
                }" v-if="item.status">{{ statusList[item.status].name }}</text>
              </view>
              ...
              <uni-row class="item-list-content">
                <uni-col :span="5" class="left">到期状态</uni-col>
                <uni-col :span="19">
                  <text v-if="item.fail_status == 0">进行中</text>
                  <text v-else-if="item.fail_status == 1">未开始</text>
    ...
    <empty v-else :index="7" title="暂无合同数据" class="bgf" style="height: calc(100vh - 300rpx);"></empty>
    	<!-- 新增 -->
		<!-- <view class="add">
