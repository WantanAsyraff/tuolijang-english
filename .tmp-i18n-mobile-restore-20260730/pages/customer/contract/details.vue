        ...
        <view class="details-content plr10">
          <view class="company-name line1 mb12">
            ...
            <text
              ...
              >{{ data.detail.contract_status.name }}</text
            >
          </view>
          <view class="info-content display-align mb12"> 订单编号：{{ data.detail.contract_no || '--' }} </view>
          <view class="info-content display-align mb12"> 客户名称：{{ data.detail.customer_name || '--' }} </view>

          <view class="info-content">
            订单金额：{{ data.detail.price || '--' }}
            <!-- <text class="line" />
            起止时间：
...
<template v-else>--</template> -->
    ...
    <view class="customer">
      ...
      <view v-if="data.currentId === 7" class="bgf pb60">
        ...
        <view v-else class="bgf">
          <view class="addSing" @click="addSign()"><text class="iconfont icon-biaodan-chengyuantianjia" />添加</view>

          <empty :index="7" title="暂无合同数据" class="bgf" style="height: calc(100vh - 300rpx)"></empty>
        </view>
      </view>
      ...
      <view v-if="data.currentId === 3" :class="data.paymentList.length > 1 ? 'pb60' : ''">
        ...
        <view class="search-box">
          ...
          <view class="add" @click="bottomBtnList(2)"><text class="iconfont icon-biaodan-chengyuantianjia" />添加</view>
        </view>
        <payment-record class="bgf plr30" :list-data="data.paymentList" :count="data.paymentCount" :cid="data.id"></payment-record>
        <view class="footer-text" v-if="data.paymentList.length > 0 && data.paymentCount <= data.paymentList.length"> -没有更多了- </view>
      </view>
      ...
      <view v-if="data.currentId === 2" :class="data.remindList.length > 1 ? 'pb60' : ''">
        <view class="search-box bgf">
          ...
          <view class="add" @click="bottomBtnList(1)"><text class="iconfont icon-biaodan-chengyuantianjia" />添加</view>
        </view>
        <pay-remind class="bgf plr30" :list-data="data.remindList" :buildData="data.buildData"></pay-remind>
        <view class="footer-text" v-if="data.remindList.length > 0 && data.remindCount <= data.remindList.length"> -没有更多了- </view>
      </view>
      ...
      <view v-if="data.currentId === 4" :class="data.invoiceList.length > 1 ? 'pb60' : ''">
        ...
        <view class="footer-text" v-if="data.invoiceList.length > 0 && data.invoiceCount <= data.invoiceList.length"> -没有更多了- </view>
      </view>
      ...
      <view v-if="data.currentId === 5">
        <view class="search-box">
          <view />
          <view class="add" @click="bottomBtnList(4)"><text class="iconfont icon-biaodan-chengyuantianjia" />添加</view>
        </view>
        <file-record
          class="bgf plr30"
    ...
    <view class="details-fixed-btn">
      ...
      <view class="btn-line" />
      <view @click="bottomBtnList(2)" class="box"><text class="iconfont icon-tianjiahuikuan"></text>添加账目</view>
      <template v-if="data.customer_switch.invoice_module_switch">
        <view class="btn-line" />
        <view @click="bottomBtnList(3)" class="box"><text class="iconfont icon-shenqingfapiao" />申请发票</view>
      </template>
    </view>
    <!--添加弹窗-->
