<template>
  <view class="form-card" :class="{ empty: productListData.length === 0 }">
    <view class="form-title">
      产品清单
      <navigator hover-class="none" :url="addProductUrl" class="add-btn">添加</navigator>
    </view>
    <view class="product-list-box">
      <ProductList :list-data="productListData" :moreShow="false">
        <template #action="{ index }">
          <view class="action-box">
            <view class="action-item" @click="handleDelProduct(index)">删除</view>
            <view class="action-item edit" @click="handleGoEditProductPage(index)">编辑</view>
          </view>
        </template>
      </ProductList>
    </view>
