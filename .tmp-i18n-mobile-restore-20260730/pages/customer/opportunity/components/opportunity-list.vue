        ...
        <template v-slot:body>
          <view class="item-list">
            <view @click="examineList(item)" class="p24">
              <view class="item-list-top">
                ...
                <text class="status-tag" :style="{
                  ...
                  background: item.status.color
                    ...
                    : getColor('#1890ff', '0.1')
                }" v-if="item.status">{{ item.status.name }}</text>
              </view>
              <uni-row class="item-list-content">
                <uni-col :span="5" class="left">商机编号</uni-col>
                <uni-col :span="19">{{ item.odds_no || '--' }}</uni-col>
              </uni-row>
              <uni-row class="item-list-content">
                <uni-col :span="5" class="left">企微客户</uni-col>
                <uni-col :span="19">
                  <view v-if="item.work_customer&&item.work_customer.name" style="display: flex; align-items: center;">
                    ...
                    <text class='work-icon over-text'
                      :class="item.work_customer.type != 1 ? 'work-name' : ''">{{ item.work_customer.type == 1 ? '@微信' : '@' + item.work_customer.corp_name || '--' }}</text>
                  </view>
                  <view v-else>
                    --
                  </view>
              ...
              <uni-row class="item-list-content">
                <uni-col :span="5" class="left">客户名称</uni-col>
                <uni-col :span="19">{{ item.odds_customer || '--' }}</uni-col>
              </uni-row>
              <uni-row class="item-list-content">
                <uni-col :span="5" class="left">商机报价</uni-col>
                <uni-col :span="19">
                  {{ item.total_amount }}元
                </uni-col>
              </uni-row>
              ...
              <uni-row class="item-list-content">
                <uni-col :span="5" class="left">跟进时间</uni-col>
                <uni-col :span="19">
                  <uni-dateformat v-if="item.last_follow_time" format="yyyy/MM/dd hh:mm"
                    :date="item.last_follow_time"></uni-dateformat>
                  <text v-else>--</text>

                  <view class="chart" @click.stop="openCustomerChat(item.work_customer)" v-if="isWxWorkEnv && item.work_customer&&item.work_customer.name">发起聊天</view>
                </uni-col>
              </uni-row>
            </view>
          </view>
