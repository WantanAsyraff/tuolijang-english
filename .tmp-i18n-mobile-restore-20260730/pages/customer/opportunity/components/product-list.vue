<template>
  <view :class="listData.length > 2 ? 'pb60' : ''">
    <view class="product-list-wrap">
            ...
            <view class="product-info-box">
              <view class="product-info-top">
                <view class="product-info">
                  ...
                  <view class="product-spec line1">{{ item.sku }} 共{{ item.count }}件</view>
                </view>
                <view class="product-right">
                  <view class="product-price">
                    <BasePriceFormat :price="getTotal(item)" />
                  </view>
                  <view class="product-count">成交总价</view>
                </view>
              </view>
              <view class="product-meta">
                <text>售价: {{ item.ot_price }}</text>
                <text>折扣: {{ item.discount }}%</text>
                <text>成交单价: {{ item.price }}</text>
              </view>
            </view>
          </view>
      ...
      <empty v-else :index="7" title="暂无产品数据"></empty>
    </view>
    <view v-if="listData.length > 0 && moreShow" class="footer-text">-没有更多了-</view>
  </view>
</template>
