  ...
  <view class="content">
        ...
        <view class="detail-banner" v-if="data.detailData">
          <view class="banner-content">
            ...
            <view
              ...
              >{{ data.detailData.data.status.name || '--' }}
            </view>
          </view>

          <view class="customer-info"> 客户名称：{{ data.detailData.data.customer_name || '--' }} </view>
          <view class="customer-info"> 商机编号：{{ data.detailData.data.odds_no || '--' }} </view>
          <view class="customer-info">
            商机报价：<text class="default-warning">{{ Number(data.detailData.data.price).toFixed(2) }}元</text>
    ...
    <view class="basic-info">
      ...
      <view v-if="data.currentId === 4" :class="data.contractList.length > 2 ? 'pb60' : ''">
        ...
        <view v-if="data.contractList.length > 0 && data.count <= data.contractList.length" class="footer-text">-没有更多了- </view>
      </view>
    </view>

    <!-- 底部按钮 -->
    <view class="details-fixed-btn">
      <view @click="AddContract" class="box"><text class="iconfont icon-guanliandingdan"></text>生成订单</view>
      <view class="btn-line" />
      <template v-if="isWxWorkEnv">
        <view @click="openCustomerChat" class="box"><text class="iconfont icon-quliaotian" />去聊天</view>
        <view class="btn-line" />
      </template>
      <view @click="AddSigning" class="box"><text class="iconfont icon-tianjiahetong1"></text>生成合同</view>
    </view>
