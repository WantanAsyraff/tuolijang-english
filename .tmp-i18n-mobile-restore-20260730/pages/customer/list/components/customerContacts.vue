<template>
  <view>
        ...
        <view class="contacts-info-item" v-for="(item, index) in contactsData" :key="index">
          <view class="contacts-header">
            ...
            <dean-popover ref="deanPopoverRef" model-direction="right">
              ...
              <view class="modal-item" @click="changePopover(item, index, 1)"><text class="iconfont icon-gongzuohuibao-bianji"></text>编辑 </view>
              <view class="modal-item" @click="changePopover(item, index, 2)"><text class="iconfont icon-shanchu1"></text>删除 </view>
            </dean-popover>
          </view>
    ...
    <view class="footer-text" v-if="contactsData.length > 0 && count <= contactsData.length">-没有更多了-</view>
  </view>
</template>
