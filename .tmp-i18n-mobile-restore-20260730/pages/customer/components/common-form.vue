<template>
  <view class="form-base">
    ...
    <view class="form-item" v-for="item of formData" :key="item.key" :type="item.type">
      <view class="form-item-label" :required="item.required">{{ item.label }}</view>
      <view class="form-item-content">
        <template v-if="item.type === 'input'">
          <input :placeholder="item.placeholder" v-model="item.value" :type="(item as FormItemInput).inputType" />
        </template>
        <template v-else-if="item.type === 'select'">
          <picker :range="(item as FormItemWithOptions).options" @change="handleSelectChange(item, $event)"
            range-key="name">
            <view class="picker-content">
              <template v-if="optionalData.selectText[item.key]">{{ optionalData.selectText[item.key] }}</template>
        ...
        <template v-else-if="item.type === 'address'">
          <uni-data-picker :localdata="optionalData.address" popup-title="请选择省市区"
        ...
        <template v-else-if="item.type === 'textarea'">
          <textarea :placeholder="item.placeholder" v-model="item.value" />
        </template>
      </view>
    </view>
  </view>
