    ...
    <view class="examine-content">
      <view class="search">
        <uni-easyinput prefixIcon="search" v-model="data.where.keyword_default" @confirm="getTableList(1)"
          @focus="getTableList(1)" @clear="clearSearch" placeholder="关键字搜索"></uni-easyinput>
      </view>
      ...
      <dean-popover v-if="data.list.length > 5" ref="deanPopoverRef" model-direction="right" :btnList="data.hide_list"
        ...
        <template #icon>
          <view class="tab-item">
            ...
            <view class="tab_text" :style="{ color: data.currentIndex == index ? selectedColor : color }">更多</view>
          </view>
        </template>
      </dean-popover>
    </view>
