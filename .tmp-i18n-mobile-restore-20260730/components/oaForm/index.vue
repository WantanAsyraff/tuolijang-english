<template>
  <view class="content">
            ...
            <view v-for="(val, index) in item.data" :key="index" class="line-box">
              ...
              <uni-forms-item class="input-label" required name="name" v-if="val.input_type === 'input' && val.type === 'text'">
                <template v-slot:label>
                  <view class="uni-forms-item__label">{{ val.key_name }} <text class="iconfont" v-if="val.required == 1">*</text></view>
                </template>
                <uni-easyinput
                  :inputBorder="false"
              ...
              <!-- <uni-forms-item class="input-label" v-if="val.input_type === 'select' && val.options_level > 1 && val.type == 'single'">
                ...
                <uni-data-picker
                  v-model="formData[val.key]"
                  :localdata="val.options"
                  :popup-title="val.placeholder || '请选择'"
              ...
              <uni-forms-item v-if="val.input_type === 'select' && !['clue_id', 'customer_label'].includes(val.key)" class="input-label">
                ...
                <view v-if="val.type == 'multiple'">
                  ...
                  <view v-if="formData[val.key]" class="picker-input" @click="openSelect(val, indexItem, index)">
                    <text class="picker-input-text">{{ val.text }}</text>
                    <view class="iconfont icon-fanhui"></view>
                  </view>
                  <multiplePicker
                    v-if="data.show"
                ...
                <template v-if="val.type === 'single'">
                  ...
                  >
                    ...
                    <view v-if="formData[val.key]" class="picker-input" :style="{ color: isReadonlyField(val) ? '#909399' : '' }">
                      <text class="picker-input-text">{{ val.text || val.text1 }}</text>
                      <view class="iconfont icon-fanhui"></view>
                    </view>
                  </picker>
              ...
              <uni-forms-item class="is-direction-top" v-if="val.type === 'textarea'">
                <template v-slot:label>
                  <view class="uni-forms-item__label mt36 mb10"> {{ val.key_name }}<text class="iconfont" v-if="val.required == 1">*</text> </view>
                </template>
                <uni-easyinput
                  :inputBorder="false"
                  v-model="formData[val.key]"
                  ...
                  :maxlength="256"
                  :placeholder="val.placeholder || '请填写' + val.key_name"
                >
                </uni-easyinput>
              </uni-forms-item>
              ...
              <uni-forms-item class="is-direction-top height-350" v-if="val.type === 'oaWangeditor'">
                <template v-slot:label>
                  <view class="uni-forms-item__label mt36"> {{ val.key_name }}<text class="iconfont" v-if="val.required == 1">*</text> </view>
                </template>
    ...
    <selected-label v-if="labelShow" title="客户标签" ref="selectedLabelRef" @changeItem="changeItem" @resetLabel="resetLabel"> </selected-label>
    <success-popup ref="successPopupRef" :type="0" title="客户" button-title="添加订单" @change="successChange"> </success-popup>
  </view>
</template>
<script setup>
