...
class AssessTrendTool extends BaseTool
{
    ...
    public function getInputSchema(): array
    {
        return [
            ...
            'properties' => array_merge([
                ...
                'types' => ['type' => 'integer', 'description' => '趋势类型：0=自己，1=下级，2=人事范围'],
    ...
    public function execute(array $arguments): array
    {
        ...
        $where = [
            ...
            'period' => (int) ($arguments['period'] ?? 2),
            'time'   => $this->timeRange($arguments) ?: date('Y/01/01') . '-' . date('Y/12/31'),
            'uid'    => $requestedUids
                ? ($allowedUids ? array_values(array_intersect($requestedUids, $allowedUids)) : $requestedUids)
                : ($this->isAdmin() ? [$userId] : ($allowedUids ?: [$userId])),
        ];

        return $service->getAssessCensusLine($where, $userId);
    }
}
