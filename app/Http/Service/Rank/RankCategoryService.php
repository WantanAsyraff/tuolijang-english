...
/**
 * 直接类别
 * Class RankCategoryService.
 */
class RankCategoryService extends BaseService implements ResourceServicesInterface
{
    ...
    /**
     * RankCategoryService constructor.
     */
    ...
    public function resourceEdit(int $id, array $other = []): array
    {
        ...
        return $this->createElementForm('修改职级类别', $this->getRankCateFormRule(collect($rankCateInfo->toArray())), '/ent/rank_cate/' . $id, 'PUT');
    }
    ...
    {
        return $this->createElementForm('添加职级类别', $this->getRankCateFormRule(collect([])), '/ent/rank_cate');
    }
    ...
    {
        $service = app()->get(RankService::class);
        if ($service->count(['cate_id' => $id])) {
            throw $this->exception('请先删除关联职级,再尝试删除');
        }
        if (app()->get(RankRelationService::class)->exists(['cate_id' => $id, 'entid' => 1])) {
    ...
    {
        return [
            Form::input('name', '职级类别名称', $collection->get('name'))->required(),
        ];
    }
}
