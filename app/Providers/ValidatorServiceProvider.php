...
class ValidatorServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        // 文本长度确认
        $validator->extend('text_strlen_confirm_api', function ($attribute, $value, $parameters) use ($toString) {
            $text = $toString($value);
            $len  = $toString($parameters[0] ?? null);

            if ($text === null || $len === null) {
                return false;
            }

            return text_strlen_confirm_api_check($text, $len);
        });
    }
}
