...
/**
 * 替换客户标签
 * Class ReplaceClientLabelUpdateJob.
 */
class ReplaceClientLabelUpdateJob implements ShouldQueue
{
    use Dispatchable;
    ...
    public function __construct(protected array $labelInfo, protected int $replaceLabelId, protected int $page = 1, protected int $limit = 50)
    {
        ...
        $this->customerService = app()->get(CustomerService::class);
    }
    ...
    {
        $list   = $this->customerService->select(['customer_label' => $this->labelInfo['id']], ['id', 'external_userid', 'customer_label', 'userid'], page: $this->page, limit: $this->limit);
        ...
        if (count($list) == $this->limit) {
            ++$this->page;
            ReplaceClientLabelUpdateJob::dispatch($this->labelInfo, $this->replaceLabelId, $this->page, $this->limit);
        }
    }
}
